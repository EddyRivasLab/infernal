...
int main(int argc, char **argv) {
  ...
  printf("FORCED J-mode: status=%d sc=%f mode=%d (nodes=%d)\n", status, sc, mode, tr?tr->n:-1);

  Parsetree_t *tr2 = NULL; char mode2 = TRMODE_UNKNOWN; float sc2 = IMPOSSIBLE;
  status = cm_TrAlignHB(cm, errbuf, dsq, L, 40000., TRMODE_R, pass_idx,
                         FALSE, FALSE, mx, shmx, NULL, NULL, NULL, NULL, &tr2, &mode2, NULL, &sc2);
  printf("FORCED R-mode: status=%d sc=%f mode=%d (nodes=%d)\n", status, sc2, mode2, tr2?tr2->n:-1);
