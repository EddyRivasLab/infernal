 ...
 *   struct hmmer_fancyali_s *ali;        // (optional structure) alignment of a hit 
 *   
...
FreeTophits(struct tophit_s *h)
{
  ...
  for (pos = 0; pos < h->num; pos++)
    {
      if (h->unsrt[pos].ali  != NULL) hmmer_FreeFancyAli(h->unsrt[pos].ali);
...
hmmer_AllocFancyAli(void)
{
  struct hmmer_fancyali_s *ali;

  ali = MallocOrDie (sizeof(struct hmmer_fancyali_s));
...
void
hmmer_FreeFancyAli(struct hmmer_fancyali_s *ali)
{
  if (ali != NULL) {
...
TophitsReport(struct tophit_s *h, double E, int nseq)
{
  ...
  for (i = 0; i < h->num; i++)
	...
	{
	  memused += sizeof(struct hmmer_fancyali_s);
	  x = 0;
