 ...
 * 
 * SVN $Id: hmmcalibrate.c 1377 2005-04-26 13:06:34Z eddy $
 */
		 ...
		 struct histogram_s **ret_hist, float *ret_max)
{
  ...
  for (idx = 0; idx < nsample; idx++)
  {
      ...
      /* generate it */
      seq = RandomSequence(hmmer_Alphabet, randomseq, Alphabet_size, sqlen);
      dsq = hmmer_DigitizeSequence(seq, sqlen);
...
worker_thread(void *ptr)
{
  ...
  for (;;)
	...
	Die("pthread_mutex_unlock failure: %s\n", strerror(rtn));
       ...
       */
      dsq = hmmer_DigitizeSequence(seq, len);
