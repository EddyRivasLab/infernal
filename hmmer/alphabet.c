...
char  hmmer_Degenerate[MAXCODE][HMMERMAXABET]; 
/* from hmmer_structs.h */
char  hmmer_Alphabet[MAXCODE+1]; /* "ACDEFGHIKLMNPQRSTVWYBZX" for example */
/* from hmmer_structs.h */
...
SetAlphabet(int type)
{
  ...
  */
  ...
  case hmmAMINO: 
    Alphabet_type     = type;
    strcpy(hmmer_Alphabet, "ACDEFGHIKLMNPQRSTVWYUBZX");
    Alphabet_size     = 20; 
    Alphabet_iupac    = 24;
    for (x = 0; x < Alphabet_iupac; x++) {
      memset(hmmer_Degenerate[x], 0, Alphabet_size);
    }
    for (x = 0; x < Alphabet_size; x++) {
      hmmer_Degenerate[x][x] = 1;
      hmmer_DegenCount[x] = 1;
    }
  ...
  case hmmNUCLEIC:
    Alphabet_type     = type;
    strcpy(hmmer_Alphabet, "ACGTUNRYMKSWHBVDX");
    Alphabet_size     = 4; 
    Alphabet_iupac    = 17;
    for (x = 0; x < Alphabet_iupac; x++) {
      memset(hmmer_Degenerate[x], 0, Alphabet_size);
 ...
 */ 
unsigned char
hmmer_SymbolIndex(char sym)
{
  char *s;
  //printf("in hmmer_Symbol index hmmer_Alphabet is %s\n", hmmer_Alphabet);
  //printf("sym is %c\n", sym);
  return ((s = strchr(hmmer_Alphabet, (char) toupper((int) sym))) == NULL) ?
	  Alphabet_iupac-1 : s - hmmer_Alphabet;
} 
 ...
 */
unsigned char *
hmmer_DigitizeSequence(char *seq, int L)
{
  unsigned char *dsq;
  int i;

  //printf("in hmmer_DigitizeSequence\n");
  //printf("seq to digitize is %s\n", seq);
  dsq = MallocOrDie (sizeof(unsigned char) * (L+2));
  dsq[0] = dsq[L+1] = (unsigned char) Alphabet_iupac;
  for (i = 1; i <= L; i++) 
    dsq[i] = hmmer_SymbolIndex(seq[i-1]);
  //printf("dedigitized seq to return is %s\n", DedigitizeSequence(dsq, L));
  return dsq;
}
 ...
 */ 
void
hmmer_DigitizeAlignment(MSA *msa, unsigned char ***ret_dsqs)
{
  unsigned char **dsq;
  ...
  for (idx = 0; idx < msa->nseq; idx++) {
	...
	dsq[idx][dpos++] = hmmer_SymbolIndex(msa->aseq[idx][apos]);
    }
...
P7CountSymbol(float *counters, unsigned char symidx, float wt)
{
  ...
  if (symidx < Alphabet_size) 
    ...
    for (x = 0; x < Alphabet_size; x++) {
      if (hmmer_Degenerate[symidx][x])
	counters[x] += wt / (float) hmmer_DegenCount[symidx];
    }
}
...
{
  hmmer_DegenCount[strchr(hmmer_Alphabet,iupac)-hmmer_Alphabet] = strlen(syms);
  while (*syms) {
    hmmer_Degenerate[strchr(hmmer_Alphabet,iupac)-hmmer_Alphabet]
              [strchr(hmmer_Alphabet,*syms)-hmmer_Alphabet] = 1;
    syms++;
  }
}
