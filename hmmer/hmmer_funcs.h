...
/* funcs.h 
 * CVS $Id: funcs.h 1377 2005-04-26 13:06:34Z eddy $
 *
 * Declarations of external functions in HMMER.
 */            

#ifndef HMMER_FUNCSH_INCLUDED
#define HMMER_FUNCSH_INCLUDED
...
extern unsigned char  hmmer_SymbolIndex(char sym);
extern unsigned char *hmmer_DigitizeSequence(char *seq, int L);
extern char          *DedigitizeSequence(unsigned char *dsq, int L);
extern void           hmmer_DigitizeAlignment(MSA *msa, unsigned char ***ret_dsqs);
 ...
 */
extern char *hmmer_Statetype(char st);
...
extern void Plan7SetNullModel(struct plan7_s *hmm, float null[HMMERMAXABET], float p1);
...
extern void P7PriorifyEmissionVector(float *vec, struct p7prior_s *pri, 
				     ...
				     float e[MAXDCHLET][HMMERMAXABET],
				     float *ret_mix);
...
extern struct hmmer_fancyali_s *hmmer_AllocFancyAli(void);
extern void   hmmer_FreeFancyAli(struct hmmer_fancyali_s *ali);
...
extern struct hmmer_fancyali_s *hmmer_CreateFancyAli(struct p7trace_s *tr, struct plan7_s *hmm,
					 unsigned char *dsq, char *name);
extern void hmmer_PrintFancyAli(FILE *fp, struct hmmer_fancyali_s *ali);
