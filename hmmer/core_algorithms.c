 ...
 * SVN $Id: core_algorithms.c 1376 2005-04-24 23:31:54Z eddy $
 * 
...
P7WeeViterbi(unsigned char *dsq, int L, struct plan7_s *hmm, struct p7trace_s **ret_tr)
{
  ...
  for (i = 1; i <= L; i++)
	...
	break;
      ...
      default: Die("Bogus state %s", P7Statetype(tassign[i]));
      }
    }
				/* terminate the trace */
  tr->statetype[tpos] = STT;
			  ...
			  int                 hmmpfam_mode)
{
  ...
  struct hmmer_fancyali_s *ali;       /* alignment of a domain      */ 
  ...
  for (tidx = 0, didx = 1; tidx < ntr; tidx++) {
    ...
    if (pvalue <= thresh->domE && score[tidx] >= thresh->domT) {
      ali     = hmmer_CreateFancyAli(tarr[tidx], hmm, dsq, seqname);
