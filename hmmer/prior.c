...
/* from sqfuncs.h APPENDED TO BOTTOM OF THIS FILE!*/
float aafq[20] = {
  0.075520,			/* A */
  0.016973,			/* C */
  0.053029,			/* D */
...
PAMPrior(char *pamfile, struct p7prior_s *pri, float wt)
{
  ...
  for (xi = 0; xi < Alphabet_size; xi++)
    for (xj = 0; xj < Alphabet_size; xj++)
      {
        idx1 = hmmer_Alphabet[xi] - 'A';
        idx2 = hmmer_Alphabet[xj] - 'A';
...
P7PriorifyEmissionVector(float *vec, struct p7prior_s *pri, 
		       int num, float eq[MAXDCHLET], float e[MAXDCHLET][HMMERMAXABET],
		       float *ret_mix)
{
