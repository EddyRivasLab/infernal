 ...
 * 
 * CVS $Id: structs.h 1376 2005-04-24 23:31:54Z eddy $
 */

#ifndef HMMER_STRUCTSH_INCLUDED
#define HMMER_STRUCTSH_INCLUDED
...
/* EPN 10.21.05 added the IMPOSSIBLE definition, yanked
   from Infernal's structs.h, needed by sreLOG2() if we
   want to use it in both Infernal and HMMER functions */

#define IMPOSSIBLE  -1e36

/* Miscellaneous math macros used in the package
 */
/* EPN 10.21.05 this is the default sreLOG2(x) line below:
   #define sreLOG2(x)  ((x) > 0 ? log(x) * 1.44269504 : -9999.)*/
/* This is the line from infernal's structs.h below: */
 ...
 */
#define SYMIDX(x)   (strchr(hmmer_Alphabet, (x)) - hmmer_Alphabet)
 ...
 * 
 * MAXCODE and HMMERMAXABET constants are defined in config.h
 */   
extern char  hmmer_Alphabet[MAXCODE+1]; /* "ACDEFGHIKLMNPQRSTVWYBZX" for example */
...
extern char  hmmer_Degenerate[MAXCODE][HMMERMAXABET];
extern int   hmmer_DegenCount[MAXCODE];
...
struct plan7_s {
   ...
   */
  float  null[HMMERMAXABET];         /* "random sequence" emission prob's     +*/
...
struct p7prior_s {
  ...
  float m[MAXDCHLET][HMMERMAXABET];  /* match emission terms per mix component    */

  int   inum;			/* number of insert emission Dirichlet mixes */
  float iq[MAXDCHLET];		/* probabilities of inum components          */
  float i[MAXDCHLET][HMMERMAXABET];	/* insert emission terms                     */
};
...
struct hit_s {
  ...
  struct hmmer_fancyali_s  *ali;	/* ptr to optional alignment info   */
};
...
struct basic_state {
  ...
  float p[HMMERMAXABET];            	/* symbol emission probabilities         */
};
...
struct plan9_s {
  ...
  float  null[HMMERMAXABET];         /* the *suggested* null model         */
