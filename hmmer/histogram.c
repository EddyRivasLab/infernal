 ...
 *
 * SVN $Id: histogram.c 1376 2005-04-24 23:31:54Z eddy $
 ************************************************************
 * Basic API:
 * 
...
double     hmmer_IncompleteGamma(double a, double x);
/* from squid's Dirichlet.h named (IncompleteGamma() APPENDED TO BOTTOM OF THIS FILE!*/
double     hmmer2_Gammln(double x);
			 ...
			 float lowbound, float highbound, int ndegrees)
{
   ...
   */
  if (nbins > 1 + ndegrees)
    h->chip = (float) hmmer_IncompleteGamma((double)(nbins-1-ndegrees)/2., 
				      (double) h->chisq/2.);
  else
    h->chip = 0.;		
}
...
GaussianFitHistogram(struct histogram_s *h, float high_hint)
{
  ...
  if (nbins > 3)
    h->chip = (float) hmmer_IncompleteGamma((double)(nbins-3)/2., 
				      (double) h->chisq/2.);
  else
    h->chip = 0.;		
...
GaussianSetHistogram(struct histogram_s *h, float mean, float sd)
{
  ...
  if (nbins > 1)
    h->chip = (float) hmmer_IncompleteGamma((double)(nbins-1)/2., 
				      (double) h->chisq/2.);
  else
    h->chip = 0.;		
}
...
/* Function: hmmer_IncompleteGamma()
 * 
 ...
 *                  p = hmmer_IncompleteGamma(v/2., x/2.) 
 ...
 */          
double
hmmer_IncompleteGamma(double a, double x)
{
  int iter;			/* iteration counter */

  if (a <= 0.) Die("hmmer_IncompleteGamma(): a must be > 0");
  if (x <  0.) Die("hmmer_IncompleteGamma(): x must be >= 0");
  ...
  if (x > a+1) 
      ...
      for (iter = 1; iter < 100; iter++)
	{
	  ...
	  if (fabs((nu1-oldp)/nu1) < 1.e-7)
	    return nu1 * exp(a * log(x) - x - hmmer2_Gammln(a));

	  oldp = nu1;
	}
      Die("hmmer_IncompleteGamma(): failed to converge using continued fraction approx");
    }
  else /* x <= a+1 */
    {
      ...
      for (iter = 1; iter < 10000; iter++)
	{
	  ...
	  if (fabs(val/p) < 1.e-7)
	    return 1. - p * exp(a * log(x) - x - hmmer2_Gammln(a));
	}
      Die("hmmer_IncompleteGamma(): failed to converge using series approx");
    }
  /*NOTREACHED*/
  return 0.;
}
