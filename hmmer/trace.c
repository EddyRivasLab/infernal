 ...
 * SVN $Id: trace.c 1377 2005-04-26 13:06:34Z eddy $
 * 
  ...
  for (tpos = 0; tpos < tr->tlen; tpos++)
	...
	default:
	  Die("illegal state transition %s->%s in traceback", 
	      P7Statetype(tr->statetype[tpos]),
	      P7Statetype(tr->statetype[tpos+1]));
	}
	break;
      case STB:
	switch (tr->statetype[tpos+1]) {
 ...
 *           cs         - p7struct_s->cs with SS cons 0..M for each node 
 *
...
P7Traces2Alignment(unsigned char **dsq, SQINFO *sqinfo, float *wgt, int nseq, int mlen, 
		   struct p7trace_s **tr, int matchonly, char *cs) 
{
  ...
  for (idx = 0; idx < nseq; idx++) {
	...
	Die("Traces2Alignment reports unrecognized statetype %c", 
	    P7Statetype(tr[idx]->statetype[tpos]));
      }
    }
  }
  ...
  for (idx = 0; idx < nseq; idx++) {
    ...
    for (tpos = 0; tpos < tr[idx]->tlen; tpos++) {
      ...
      if (statetype == STM) {
	apos = matmap[k];
	msa->aseq[idx][apos] = hmmer_Alphabet[dsq[idx][rpos]];
	apos++;
      }
	...
	if (matchonly) 
	  ...
	  msa->aseq[idx][apos] = (char) tolower((int) hmmer_Alphabet[dsq[idx][rpos]]);
	  apos++;
	}
      }
  ...
  msa->nseq = nseq;
  msa->alen = alen;
  msa->au   = MallocOrDie(sizeof(char) * (strlen(HMMER_PACKAGE_VERSION)+7));
  sprintf(msa->au, "HMMER %s", HMMER_PACKAGE_VERSION);
  ...
  /* EPN - 05.03.05 A cs string is passed into this function, that
   *       allows the output alignment from hmmbuild to have a 
   *       #=GC SS_cons line if one was supplied in the input
   *       alignment.  This is useful for determining accuracy of
   *       an HMM at structural RNA alignment.  If there was no 
   *       #=GC SS_cons markup in the original input file, then
   *       no msa->ss_cons is written, and no SS_cons is produced
   *       in the resulting Stockholm file outputted from hmmbuild.
   */
		      ...
		      char st2, int k2)
{
  ...
  case STN:
    ...
    default:      Die("illegal %s->%s transition", P7Statetype(st1), P7Statetype(st2));
    }
    break;
  case STB:
    switch (st2) {
  ...
  default:        Die("illegal state %s in traceback", P7Statetype(st1));
  }
  /*NOTREACHED*/
  return 0;
}
 ...
 */
struct hmmer_fancyali_s *
hmmer_CreateFancyAli(struct p7trace_s *tr, struct plan7_s *hmm,
	       unsigned char *dsq, char *name)
{
  struct hmmer_fancyali_s *ali;       /* alignment to create                */
  ...
  for (tpos = 0; tpos < tr->tlen; tpos++) {
    ...
    case STC:
      ali->model[tpos] = '-';
      if (tr->pos[tpos] > 0) { 
	ali->aseq[tpos] = tolower(hmmer_Alphabet[dsq[tr->pos[tpos]]]);
      }
      break;
	...
	ali->mline[tpos] = '+';
      ali->aseq[tpos]  = (char) tolower((int) hmmer_Alphabet[dsq[tr->pos[tpos]]]);
      break;
 ...
 */
void
hmmer_PrintFancyAli(FILE *fp, struct hmmer_fancyali_s *ali)
{
