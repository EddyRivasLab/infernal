		...
		struct plan7_s **ret_hmm, struct p7trace_s  ***ret_tr)
{
  ...
  float   matp[HMMERMAXABET];        /* match emission vector           */
  float   insp[HMMERMAXABET];	/* insert score vector             */
  float   insc[HMMERMAXABET];	/* insert count vector             */
  ...
  for (i = 0; i < msa->alen; i++) 
	...
	P7CountSymbol(matc[i+1], hmmer_SymbolIndex(msa->aseq[idx][i]), msa->wgt[idx]);
  mpri = sreLOG2(mpri);
