...
% using banded dynamic programming}
...
probabilistic bounds on which cells of the CYK dynamic programming matrix
...
of RNA secondary structure; a special case of profile stochastic
context-free grammars (profile SCFGs), analogous to profile hidden
Markov models of linear sequence. 
The dynamic programming algorithm for searching a database of $L$
residues with a query RNA structure of $N$ residues is $O(N^3
L)$. Poor search speed is a major factor limiting practical
applications of CM methods.
%Also, several effective strategies for
%increasing the sensitivity of profile HMMs have yet to be applied to
%CMs. 

\textbf{Methodology/Principal Findings:} 
To accelerate CM database search, we describe a method for precalculating
probabilistic bounds on which cells of the CYK dynamic programming
matrix need to be calculated. These bounds are calculated  \emph{a
  priori}, independent of the target database sequence. We also describe
the estimation of emission and transition prior probabilities and an
entropy weighting strategy that make CMs more sensitive for homology
detection. A new release of the \textsc{Infernal} software package,
version 0.70, which implements the techniques and algorithms discussed
here, is now freely available from our lab's website.


\textbf{Conclusions/Significance:} Preliminary results on an Rfam
based benchmark show that CMs are now significantly more sensitive and
a speed-up of 3-5X on average is achievable with the banded CYK
algorithm, without an appreciable cost in performance. 
}

\section{Introduction}

Non-coding RNA (ncRNA) genes encode functional RNA molecules that are not
translated to proteins. Our understanding of the importance and
breadth of the roles of ncRNAs in the
cell continues to grow through both discovery of novel families and
further study of known families, including comparative sequence analysis of
homologs across species. But ncRNA genes are difficult to
find relative to protein coding genes. Their small size and
immunity to frameshift mutations make them hard to
detect by mutational genetic screens, and they do not lie within open
reading frames hiding them from traditional computational
gene-finders. However, many RNA families exhibit conservation at the
structural and sequence level, and computational analyses that
consider both of these types of information have been
informative. These efforts have mainly focused
on three fundamental problems: finding novel RNAs, folding RNAs of unknown
structure, and finding homologous RNAs.

\begin{comment}
Many RNA families exhibit conservation at the structural and
sequence level. Consequently, computational analyses of RNA families
benefit from consideration of both types of
conservation. These computational efforts have mainly focused on a few

RNA genes pose
a unique problem to computational biologists 

%The known cellular roles of ncRNAs 
%continues to expand with the discovery of new families.
Discoveries of novel RNA gene families
continue to expand our understanding of their important regulatory and
catalytic roles in the cell, 

The number of known RNA families and motifs continues to grow. Many of
these families exhibit conservation at the structural and sequence
level. Consequently, computational analyses of RNA families benefit
from consideration of both secondary structure and primary sequence 
conservation \cite{Dandekar95, Eddy02}.
\end{comment}
...
%Their probabilistic nature provides a well-founded,
%mathematically rigorous formulation with dynamic programming (DP)
%algorithms for searching for and aligning homologous RNA
%sequences. 
...
are discovered,
%(either resulting from or
%resulting in more sensitive detection tools), 
we expect the observed
sequence identity within many of these families to decrease, making
filters less useful.
\begin{comment}
As detection tools become more sensitive and distant homologs are
discovered as a result of increased

sensitivity of homology detection methods, or vice versa, 
become more sensitive, or both, 
ese families become more represent
We expect that as homology detection programs, including
CM methods, become more sensitive, families 
\end{comment}
