		       ...
		       int bdump_level, int debug_level, int silent_mode, ESL_RANDOMNESS *r)
{
  ...
  if( sq_mode && (seqs_to_aln->sq       == NULL)) cm_Fail("ERROR: actually_align_targets(), in sq_mode, seqs_to_aln->sq is NULL.\n");
  ...
  if((cm->align_opts & CM_ALIGN_SCOREONLY) && (cm->align_opts & CM_ALIGN_HMMONLY))  
    cm_Fail("ERROR: actually_align_targets(), CM_ALIGN_SCOREONLY and CM_ALIGN_HMMONLY options are incompatible.\n");
  if((cm->align_opts & CM_ALIGN_SCOREONLY) && (cm->align_opts & CM_ALIGN_POST))
    cm_Fail("ERROR: actually_align_targets(), CM_ALIGN_SCOREONLY and CM_ALIGN_POST options are incompatible.\n");
  ...
  /* Optionally allocate tr, cp9_tr, and postcode, only if sqmode. We set
   * seqs_to_aln->tr, seqs_to_aln->cp9_tr, and seqs_to_aln->postcode to 
   * these guys at end of function.
   * If dsqmode, do not allocate parsetree pointers already exist 
   * in search_results.
   */
  ...
  for (i = 0; i < nalign; i++)
	...
	{
	  if(do_sub) 
	    printf("Aligning (to a sub CM) %-20s", seqs_to_aln->sq[i]->name);
	  else
