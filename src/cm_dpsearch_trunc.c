			...
			CM_TR_HB_MX *mx, float size_limit, float env_cutoff, int64_t *ret_envi, int64_t *ret_envj, float *ret_sc)
{
  ...
  for (v = cm->M-1; v >= 0; v--) { /* all the way down to root, different from other scanners */
    ...
    lpos = (cm->stid[v] == MATP_MP || cm->stid[v] == MATL_ML) ? cm->emap->lpos[cm->ndidx[v]] : cm->emap->lpos[cm->ndidx[v]]+1;
    rpos = (cm->stid[v] == MATP_MP || cm->stid[v] == MATR_MR) ? cm->emap->rpos[cm->ndidx[v]] : cm->emap->rpos[cm->ndidx[v]]-1;
	...
	if((do_J_v && do_J_y) || (do_R_v && do_R_y)) { 
	...
	if((do_J_v && do_J_y) || (do_L_v && do_L_y) || (do_R_v && do_R_y)) { 
      ...
      do_L_y = cp9b->do_L[y];
      do_R_y = cp9b->do_R[y];
      do_T_y = cp9b->do_T[y]; /* will be FALSE, not a B_st */
      do_J_z = cp9b->do_J[z];
      do_L_z = cp9b->do_L[z];
      do_R_z = cp9b->do_R[z];
      do_T_z = cp9b->do_T[z]; /* will be FALSE, not a B_st */
