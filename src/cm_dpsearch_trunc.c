 ...
 */
int
FastTrCYKScan(CM_t *cm, char *errbuf, TrScanMatrix_t *trsmx, ESL_DSQ *dsq, int i0, int j0, float cutoff, CM_TOPHITS *hitlist,
	      int do_null3, float env_cutoff, int64_t *ret_envi, int64_t *ret_envj, float **ret_vsc, float *ret_sc)
{
  ...
  float    *Jsc_v;              /* [0..d..W] temporary score vec for each d for current j & v */
  float    *Lsc_v;              /* [0..d..W] temporary score vec for each d for current j & v */
  float    *Rsc_v;              /* [0..d..W] temporary score vec for each d for current j & v */
  ...
  /* Contract check */
  if(! cm->flags & CMH_BITS)             ESL_FAIL(eslEINCOMPAT, errbuf, "FastTrCYKScan, CMH_BITS flag is not raised.\n");
  if(j0 < i0)                            ESL_FAIL(eslEINCOMPAT, errbuf, "FastTrCYKScan, i0: %d j0: %d\n", i0, j0);
  if(dsq == NULL)                        ESL_FAIL(eslEINCOMPAT, errbuf, "FastTrCYKScan, dsq is NULL\n");
  if(cm->search_opts & CM_SEARCH_INSIDE) ESL_FAIL(eslEINCOMPAT, errbuf, "FastTrCYKScan, CM_SEARCH_INSIDE flag raised");
  if(trsmx == NULL)                      ESL_FAIL(eslEINCOMPAT, errbuf, "FastTrCYKScan, trsmx == NULL\n");
  //if(! (smx->flags & cmSMX_HAS_FLOAT))   ESL_FAIL(eslEINCOMPAT, errbuf, "FastTrCYKScan, ScanMatrix's cmSMX_HAS_FLOAT flag is not raised");
  //if(smx == cm->smx && (! cm->flags & CMH_SCANMATRIX)) ESL_FAIL(eslEINCOMPAT, errbuf, "FastTrCYKScan, smx == cm->smx, and cm->flags & CMH_SCANMATRIX is down, matrix is invalid.");
  ...
  /* Initialize {J,L,R}sc_v */
  ESL_ALLOC(Jsc_v, (sizeof(float) * (W+1)));
  ESL_ALLOC(Lsc_v, (sizeof(float) * (W+1)));
  ESL_ALLOC(Rsc_v, (sizeof(float) * (W+1)));
  esl_vec_FSet(Jsc_v, (W+1), IMPOSSIBLE);
  esl_vec_FSet(Lsc_v, (W+1), IMPOSSIBLE);
  esl_vec_FSet(Rsc_v, (W+1), IMPOSSIBLE);
  ...
  for (j = i0; j <= j0; j++) 
      ...
      for (v = cm->M-1; v > 0; v--) /* ...almost to ROOT; we handle ROOT specially... */
	{
	  ...
	  if(cm->sttype[v] == B_st) {
	    ...
	    switch (cnum) {
	    case 3: 
	      /* fill Jalpha and Lalpha */
	      i = j-dn+1;  /* initial i, we decrement it when we access it */
	      Jarow0 = Jalpha[jp_y][y];
	      Jarow1 = Jalpha[jp_y][y+1];
	      Jarow2 = Jalpha[jp_y][y+2];
	      Larow0 = Lalpha[jp_y][y];
	      Larow1 = Lalpha[jp_y][y+1];
	      Larow2 = Lalpha[jp_y][y+2];
	      dp_y = dn-sd; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      /* first, deal with edge case of d == 1, Jsc calc is standard, Lsc is different */
	      assert(dn == 1);
	      d = dn;
	      Jsc = ESL_MAX(Jarow2[dp_y] + tsc_v[2],
			    Jarow1[dp_y] + tsc_v[1]);		
	      Jsc = ESL_MAX(Jsc, init_scAA[v][dp_y]);
	      Jsc = ESL_MAX(Jsc, Jarow0[dp_y] + tsc_v[0]);		
	      Jsc += esc_v[dsq[i]];
	      Jalpha[jp_v][v][d] = Jsc;
	      Lalpha[jp_v][v][d] = esc_v[dsq[i--]];
	      d++;
	      dp_y++;
	      for (; d <= dx; d++, dp_y++) {
		Jsc = ESL_MAX(Jarow2[dp_y] + tsc_v[2],
		...
		Lsc = ESL_MAX(Larow2[dp_y] + tsc_v[2],
			      Larow1[dp_y] + tsc_v[1]);		
		Lsc = ESL_MAX(Lsc, IMPOSSIBLE);
		Lsc = ESL_MAX(Lsc, Larow0[dp_y] + tsc_v[0]);		
		...
		Lalpha[jp_v][v][d] = Lsc;
	      ...
	      /* fill Ralpha, in IL case, Ralpha can depend on Jalpha calc'ed just above */
	      Rarow0 = Ralpha[jp_y][y];
	      Rarow1 = Ralpha[jp_y][y+1];
	      Rarow2 = Ralpha[jp_y][y+2];
	      dp_y = dn; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      for (d = dn; d <= dx; d++, dp_y++) {
		Ralpha[jp_v][v][d] = IMPOSSIBLE; /* this is important b/c if we're an IL, we'll access this cell in the recursion below for Ralpha */
		Rsc = ESL_MAX(ESL_MAX(Jarow2[dp_y] + tsc_v[2], Rarow2[dp_y] + tsc_v[2]),
			      ESL_MAX(Jarow1[dp_y] + tsc_v[1], Rarow1[dp_y] + tsc_v[1]));		
		Rsc = ESL_MAX(Rsc, IMPOSSIBLE);
		Rsc = ESL_MAX(Rsc, ESL_MAX(Jarow0[dp_y] + tsc_v[0], Rarow0[dp_y] + tsc_v[0]));		
		Ralpha[jp_v][v][d] = Rsc;
	      } /* end of for(d = dn; d <= dx; d++) */
	      break; /* end of 'case 3:' */
	    ...
	    case 6: 
	      ...
	      Jarow1 = Jalpha[jp_y][y+1];
	      Jarow2 = Jalpha[jp_y][y+2];
	      Jarow3 = Jalpha[jp_y][y+3];
	      Jarow4 = Jalpha[jp_y][y+4];
	      Jarow5 = Jalpha[jp_y][y+5];
	      Larow0 = Lalpha[jp_y][y];
	      Larow1 = Lalpha[jp_y][y+1];
	      Larow2 = Lalpha[jp_y][y+2];
	      Larow3 = Lalpha[jp_y][y+3];
	      Larow4 = Lalpha[jp_y][y+4];
	      Larow5 = Lalpha[jp_y][y+5];
	      dp_y = dn-sd; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      /* first, deal with edge case of d == 1, Jsc calc is standard, Lsc is different */
	      assert(dn == 1);
	      d = dn;
	      Jsc = ESL_MAX(Jarow5[dp_y] + tsc_v[5],
			    init_scAA[v][dp_y]);
	      Jsc = ESL_MAX(Jsc, Jarow4[dp_y] + tsc_v[4]);		
	      Jsc = ESL_MAX(Jsc, Jarow3[dp_y] + tsc_v[3]);		
	      Jsc = ESL_MAX(Jsc, Jarow2[dp_y] + tsc_v[2]);		
	      Jsc = ESL_MAX(Jsc, Jarow1[dp_y] + tsc_v[1]);		
	      Jsc = ESL_MAX(Jsc, Jarow0[dp_y] + tsc_v[0]);		
	      Jsc += esc_v[dsq[i]];
	      Jalpha[jp_v][v][d]  = Jsc;
	      Lalpha[jp_v][v][d] = esc_v[dsq[i--]];
		...
		Lsc = ESL_MAX(Larow5[dp_y] + tsc_v[5],
			      IMPOSSIBLE);
		Lsc = ESL_MAX(Lsc, Larow4[dp_y] + tsc_v[4]);		
		Lsc = ESL_MAX(Lsc, Larow3[dp_y] + tsc_v[3]);		
		Lsc = ESL_MAX(Lsc, Larow2[dp_y] + tsc_v[2]);		
		Lsc = ESL_MAX(Lsc, Larow1[dp_y] + tsc_v[1]);		
		Lsc = ESL_MAX(Lsc, Larow0[dp_y] + tsc_v[0]);		
		...
		Lalpha[jp_v][v][d] = Lsc;
	      ...
	      Rarow1 = Ralpha[jp_y][y+1];
	      Rarow2 = Ralpha[jp_y][y+2];
	      Rarow3 = Ralpha[jp_y][y+3];
	      Rarow4 = Ralpha[jp_y][y+4];
	      Rarow5 = Ralpha[jp_y][y+5];
	      dp_y = dn; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      for (d = dn; d <= dx; d++, dp_y++) {
		Ralpha[jp_v][v][d] = IMPOSSIBLE; /* this is important b/c if we're an IL, we'll access this cell in the recursion below for Ralpha */
		Rsc = ESL_MAX(ESL_MAX(Jarow5[dp_y] + tsc_v[5], Rarow5[dp_y] + tsc_v[5]),
			      IMPOSSIBLE);
		Rsc = ESL_MAX(Rsc, ESL_MAX(Jarow4[dp_y] + tsc_v[4], Rarow4[dp_y] + tsc_v[4]));		
		Rsc = ESL_MAX(Rsc, ESL_MAX(Jarow3[dp_y] + tsc_v[3], Rarow3[dp_y] + tsc_v[3]));		
		Rsc = ESL_MAX(Rsc, ESL_MAX(Jarow2[dp_y] + tsc_v[2], Rarow2[dp_y] + tsc_v[2]));		
		Rsc = ESL_MAX(Rsc, ESL_MAX(Jarow1[dp_y] + tsc_v[1], Rarow1[dp_y] + tsc_v[1]));		
		Rsc = ESL_MAX(Rsc, ESL_MAX(Jarow0[dp_y] + tsc_v[0], Rarow0[dp_y] + tsc_v[0]));		
		Ralpha[jp_v][v][d] = Rsc;
	      } /* end of for(d = dn; d <= dx; d++) */
	      break; /* end of 'case 6:' */
	    ...
	    case 4: 
	      ...
	      assert(dn == 1);
	      d = dn;
	      Jsc = ESL_MAX(Jarow3[dp_y] + tsc_v[3],
			    Jarow2[dp_y] + tsc_v[2]);		
	      Jsc = ESL_MAX(Jsc, init_scAA[v][dp_y]);
	      Jsc = ESL_MAX(Jsc, Jarow1[dp_y] + tsc_v[1]);		
	      Jsc = ESL_MAX(Jsc, Jarow0[dp_y] + tsc_v[0]);		
		...
		Lsc = ESL_MAX(Larow3[dp_y] + tsc_v[3],
			      Larow2[dp_y] + tsc_v[2]);		
		Lsc = ESL_MAX(Lsc, IMPOSSIBLE);
		Lsc = ESL_MAX(Lsc, Larow1[dp_y] + tsc_v[1]);		
		Lsc = ESL_MAX(Lsc, Larow0[dp_y] + tsc_v[0]);		
		...
		Lalpha[jp_v][v][d] = Lsc;
	      ...
	      for (d = dn; d <= dx; d++, dp_y++) {
		Ralpha[jp_v][v][d] = IMPOSSIBLE; /* this is important b/c if we're an IL, we'll access this cell in the recursion below for Ralpha */
		Rsc = ESL_MAX(ESL_MAX(Jarow3[dp_y] + tsc_v[3], Rarow3[dp_y] + tsc_v[3]),
			      ESL_MAX(Jarow2[dp_y] + tsc_v[2], Rarow2[dp_y] + tsc_v[2]));		
		Rsc = ESL_MAX(Rsc, IMPOSSIBLE);
		Rsc = ESL_MAX(Rsc, ESL_MAX(Jarow1[dp_y] + tsc_v[1], Rarow1[dp_y] + tsc_v[1]));		
		Rsc = ESL_MAX(Rsc, ESL_MAX(Jarow0[dp_y] + tsc_v[0], Rarow0[dp_y] + tsc_v[0]));		
		Ralpha[jp_v][v][d] = Rsc;
	    ...
	    case 5: 
	      ...
	      assert(dn == 1);
	      d = dn;
	      Jsc = ESL_MAX(Jarow4[dp_y] + tsc_v[4],
			    Jarow3[dp_y] + tsc_v[3]);		
	      Jsc = ESL_MAX(Jsc, init_scAA[v][dp_y]);
	      Jsc = ESL_MAX(Jsc, Jarow1[dp_y] + tsc_v[1]);		
	      Jsc = ESL_MAX(Jsc, Jarow2[dp_y] + tsc_v[2]);		
		...
		Lsc = ESL_MAX(Larow4[dp_y] + tsc_v[4],
			      Larow3[dp_y] + tsc_v[3]);		
		Lsc = ESL_MAX(Lsc, IMPOSSIBLE);
		Lsc = ESL_MAX(Lsc, Larow1[dp_y] + tsc_v[1]);		
		Lsc = ESL_MAX(Lsc, Larow2[dp_y] + tsc_v[2]);		
		...
		Lalpha[jp_v][v][d] = Lsc;
	      ...
	      for (d = dn; d <= dx; d++, dp_y++) {
		Ralpha[jp_v][v][d] = IMPOSSIBLE; /* this is important b/c if we're an IL, we'll access this cell in the recursion below for Ralpha */
		Rsc = ESL_MAX(ESL_MAX(Jarow4[dp_y] + tsc_v[4], Rarow4[dp_y] + tsc_v[4]),
			      ESL_MAX(Jarow3[dp_y] + tsc_v[3], Rarow3[dp_y] + tsc_v[3]));		
		Rsc = ESL_MAX(Rsc, IMPOSSIBLE);
		Rsc = ESL_MAX(Rsc, ESL_MAX(Jarow1[dp_y] + tsc_v[1], Rarow1[dp_y] + tsc_v[1]));		
		Rsc = ESL_MAX(Rsc, ESL_MAX(Jarow2[dp_y] + tsc_v[2], Rarow2[dp_y] + tsc_v[2]));		
		Rsc = ESL_MAX(Rsc, ESL_MAX(Jarow0[dp_y] + tsc_v[0], Rarow0[dp_y] + tsc_v[0]));		
	    ...
	    case 2: 
	      ...
	      assert(dn == 1);
	      d = dn;
	      Jsc = ESL_MAX(Jarow1[dp_y] + tsc_v[1], init_scAA[v][dp_y]);
	      Jsc = ESL_MAX(Jsc, Jarow0[dp_y] + tsc_v[0]);		
	      Jsc += esc_v[dsq[i]];
	      Jalpha[jp_v][v][d] = Jsc;
	      Lalpha[jp_v][v][d] = esc_v[dsq[i--]];
	      ...
	      dp_y++;
	      for (; d <= dx; d++, dp_y++) {
		Jsc = ESL_MAX(Jarow1[dp_y] + tsc_v[1],
			      init_scAA[v][dp_y]);
		Jsc = ESL_MAX(Jsc, Jarow0[dp_y] + tsc_v[0]);		

		Lsc = ESL_MAX(Larow1[dp_y] + tsc_v[1],
			      IMPOSSIBLE);
		Lsc = ESL_MAX(Lsc, Larow0[dp_y] + tsc_v[0]);		
		...
		Lalpha[jp_v][v][d] = Lsc;
	      ...
	      for (d = dn; d <= dx; d++, dp_y++) {
		Ralpha[jp_v][v][d] = IMPOSSIBLE; /* this is important b/c if we're an IL, we'll access this cell in the recursion below for Ralpha */
		Rsc = ESL_MAX(ESL_MAX(Jarow1[dp_y] + tsc_v[1], Rarow1[dp_y] + tsc_v[1]),
			      IMPOSSIBLE);
		Rsc = ESL_MAX(Rsc, ESL_MAX(Jarow0[dp_y] + tsc_v[0], Rarow0[dp_y] + tsc_v[0]));		
		Ralpha[jp_v][v][d] = Rsc;
	      } /* end of for(d = dn; d <= dx; d++) */
	    ...
	    case 3: 
	      ...
	      Rarow2 = Ralpha[jp_y][y+2];
	      dp_y = dn - sd; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      /* first, deal with edge case of d == 1, Jsc calc is standard, Rsc is different */
	      assert(dn == 1);
	      d = dn;
	      Jsc = ESL_MAX(Jarow2[dp_y] + tsc_v[2],
			    Jarow1[dp_y] + tsc_v[1]);		
		...
		Rsc = ESL_MAX(Rarow2[dp_y] + tsc_v[2],
			      Rarow1[dp_y] + tsc_v[1]);		
		Rsc = ESL_MAX(Rsc, IMPOSSIBLE);
		Rsc = ESL_MAX(Rsc, Rarow0[dp_y] + tsc_v[0]);		
		...
		Ralpha[jp_v][v][d] = Rsc;
	      ...
	      /* fill Lalpha, in IR case, Lalpha can depend on Jalpha calc'ed just above */
	      /* NOTE: we're off-by-one w.r.t J and R here, jq_y !+ jp_y */
	      Jarow0 = Jalpha[jq_y][y];
	      Jarow1 = Jalpha[jq_y][y+1];
	      Jarow2 = Jalpha[jq_y][y+2];
	      Larow0 = Lalpha[jq_y][y];
	      Larow1 = Lalpha[jq_y][y+1];
	      Larow2 = Lalpha[jq_y][y+2];
	      dp_y = dn; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      for (d = dn; d <= dx; d++, dp_y++) {
		Lalpha[jp_v][v][d] = IMPOSSIBLE; /* this is important b/c if we're an IR, we'll access this cell in the recursion below for Lalpha */
		Lsc = ESL_MAX(ESL_MAX(Jarow2[dp_y] + tsc_v[2], Larow2[dp_y] + tsc_v[2]),
			      ESL_MAX(Jarow1[dp_y] + tsc_v[1], Larow1[dp_y] + tsc_v[1]));		
		Lsc = ESL_MAX(Lsc, IMPOSSIBLE);
		Lsc = ESL_MAX(Lsc, ESL_MAX(Jarow0[dp_y] + tsc_v[0], Larow0[dp_y] + tsc_v[0]));		
		Lalpha[jp_v][v][d] = Lsc;
	      } /* end of for(d = dn; d <= dx; d++) */
	      break; /* end of 'case 3:' */
		...
		Rsc = ESL_MAX(Rarow5[dp_y] + tsc_v[5],
			      IMPOSSIBLE);
		Rsc = ESL_MAX(Rsc, Rarow4[dp_y] + tsc_v[4]);		
		Rsc = ESL_MAX(Rsc, Rarow3[dp_y] + tsc_v[3]);		
		Rsc = ESL_MAX(Rsc, Rarow2[dp_y] + tsc_v[2]);		
		Rsc = ESL_MAX(Rsc, Rarow1[dp_y] + tsc_v[1]);		
		Rsc = ESL_MAX(Rsc, Rarow0[dp_y] + tsc_v[0]);		
		...
		Ralpha[jp_v][v][d] = Rsc;
	      ...
	      Jarow1 = Jalpha[jq_y][y+1];
	      Jarow2 = Jalpha[jq_y][y+2];
	      Jarow3 = Jalpha[jq_y][y+3];
	      Jarow4 = Jalpha[jq_y][y+4];
	      Jarow5 = Jalpha[jq_y][y+5];
	      Larow0 = Lalpha[jq_y][y];
	      Larow1 = Lalpha[jq_y][y+1];
	      Larow2 = Lalpha[jq_y][y+2];
	      Larow3 = Lalpha[jq_y][y+3];
	      Larow4 = Lalpha[jq_y][y+4];
	      Larow5 = Lalpha[jq_y][y+5];
	      dp_y = dn; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      for (d = dn; d <= dx; d++, dp_y++) {
		Lalpha[jp_v][v][d] = IMPOSSIBLE; /* this is important b/c if we're an IR, we'll access this cell in the recursion below for Lalpha */
		Lsc = ESL_MAX(ESL_MAX(Jarow5[dp_y] + tsc_v[5], Larow5[dp_y] + tsc_v[5]),
			      IMPOSSIBLE);
		Lsc = ESL_MAX(Lsc, ESL_MAX(Jarow4[dp_y] + tsc_v[4], Larow4[dp_y] + tsc_v[4]));		
		Lsc = ESL_MAX(Lsc, ESL_MAX(Jarow3[dp_y] + tsc_v[3], Larow3[dp_y] + tsc_v[3]));		
		Lsc = ESL_MAX(Lsc, ESL_MAX(Jarow2[dp_y] + tsc_v[2], Larow2[dp_y] + tsc_v[2]));		
		Lsc = ESL_MAX(Lsc, ESL_MAX(Jarow1[dp_y] + tsc_v[1], Larow1[dp_y] + tsc_v[1]));		
		Lsc = ESL_MAX(Lsc, ESL_MAX(Jarow0[dp_y] + tsc_v[0], Larow0[dp_y] + tsc_v[0]));		
		Lalpha[jp_v][v][d] = Lsc;
	      } /* end of for(d = dn; d <= dx; d++) */
	      break; /* end of 'case 6:' */
		...
		Rsc = ESL_MAX(Rarow3[dp_y] + tsc_v[3],
			      Rarow2[dp_y] + tsc_v[2]);		
		Rsc = ESL_MAX(Rsc, IMPOSSIBLE);
		Rsc = ESL_MAX(Rsc, Rarow1[dp_y] + tsc_v[1]);		
		Rsc = ESL_MAX(Rsc, Rarow0[dp_y] + tsc_v[0]);		
		...
		Ralpha[jp_v][v][d] = Rsc;
	      ...
	      for (d = dn; d <= dx; d++, dp_y++) {
		Lalpha[jp_v][v][d] = IMPOSSIBLE; /* this is important b/c if we're an IR, we'll access this cell in the recursion below for Lalpha */
		Lsc = ESL_MAX(ESL_MAX(Jarow3[dp_y] + tsc_v[3], Larow3[dp_y] + tsc_v[3]),
			      ESL_MAX(Jarow2[dp_y] + tsc_v[2], Larow2[dp_y] + tsc_v[2]));		
		Lsc = ESL_MAX(Lsc, IMPOSSIBLE);
		Lsc = ESL_MAX(Lsc, ESL_MAX(Jarow1[dp_y] + tsc_v[1], Larow1[dp_y] + tsc_v[1]));		
		Lsc = ESL_MAX(Lsc, ESL_MAX(Jarow0[dp_y] + tsc_v[0], Larow0[dp_y] + tsc_v[0]));		
		Lalpha[jp_v][v][d] = Lsc;
		...
		Rsc = ESL_MAX(Rarow4[dp_y] + tsc_v[4],
			      Rarow3[dp_y] + tsc_v[3]);		
		Rsc = ESL_MAX(Rsc, IMPOSSIBLE);
		Rsc = ESL_MAX(Rsc, Rarow1[dp_y] + tsc_v[1]);		
		Rsc = ESL_MAX(Rsc, Rarow2[dp_y] + tsc_v[2]);		
		...
		Ralpha[jp_v][v][d] = Rsc;
	      ...
	      for (d = dn; d <= dx; d++, dp_y++) {
		Lalpha[jp_v][v][d] = IMPOSSIBLE; /* this is important b/c if we're an IR, we'll access this cell in the recursion below for Lalpha */
		Lsc = ESL_MAX(ESL_MAX(Jarow4[dp_y] + tsc_v[4], Larow4[dp_y] + tsc_v[4]),
			      ESL_MAX(Jarow3[dp_y] + tsc_v[3], Larow3[dp_y] + tsc_v[3]));		
		Lsc = ESL_MAX(Lsc, IMPOSSIBLE);
		Lsc = ESL_MAX(Lsc, ESL_MAX(Jarow1[dp_y] + tsc_v[1], Larow1[dp_y] + tsc_v[1]));		
		Lsc = ESL_MAX(Lsc, ESL_MAX(Jarow2[dp_y] + tsc_v[2], Larow2[dp_y] + tsc_v[2]));		
		Lsc = ESL_MAX(Lsc, ESL_MAX(Jarow0[dp_y] + tsc_v[0], Larow0[dp_y] + tsc_v[0]));		
		...
		Rsc = ESL_MAX(Rarow1[dp_y] + tsc_v[1],
			      IMPOSSIBLE);
		Rsc = ESL_MAX(Rsc, Rarow0[dp_y] + tsc_v[0]);		
		...
		Ralpha[jp_v][v][d] = Rsc;
	      ...
	      for (d = dn; d <= dx; d++, dp_y++) {
		Lalpha[jp_v][v][d] = IMPOSSIBLE; /* this is important b/c if we're an IR, we'll access this cell in the recursion below for Lalpha */
		Lsc = ESL_MAX(ESL_MAX(Jarow1[dp_y] + tsc_v[1], Larow1[dp_y] + tsc_v[1]),
			      IMPOSSIBLE);
		Lsc = ESL_MAX(Lsc, ESL_MAX(Jarow0[dp_y] + tsc_v[0], Larow0[dp_y] + tsc_v[0]));		
		Lalpha[jp_v][v][d] = Lsc;
	      } /* end of for(d = dn; d <= dx; d++) */
	  ...
	  } /* end of else if (v == MR_st || v == IR_st) */
	    ...
	    case 3: 
	      ...
	      dp_y = dn - sd; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      for (d = dn; d <= dx; d++) {
		Jsc_v[d] = ESL_MAX(Jarow2[dp_y] + tsc_v[2],
				   Jarow1[dp_y] + tsc_v[1]);		
		Jsc_v[d] = ESL_MAX(Jsc_v[d], init_scAA[v][dp_y]);
		Jsc_v[d] = ESL_MAX(Jsc_v[d], Jarow0[dp_y] + tsc_v[0]);		

		dp_y++;

		Rsc_v[d] = ESL_MAX(ESL_MAX(Jarow2[dp_y] + tsc_v[2], Rarow2[dp_y] + tsc_v[2]),
				   ESL_MAX(Jarow1[dp_y] + tsc_v[1], Rarow1[dp_y] + tsc_v[1]));		
		Rsc_v[d] = ESL_MAX(Rsc_v[d], IMPOSSIBLE);
		Rsc_v[d] = ESL_MAX(Rsc_v[d], ESL_MAX(Jarow0[dp_y] + tsc_v[0], Rarow0[dp_y] + tsc_v[0]));		
	      } /* end of for(d = dn; d <= dx; d++) */
	      ...
	      Larow1 = Lalpha[jq_y][y+1];
	      Larow2 = Lalpha[jq_y][y+2];
	      dp_y = dn - 1; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      for (d = dn; d <= dx; d++, dp_y++) {
		Lsc_v[d] = ESL_MAX(ESL_MAX(Jarow2[dp_y] + tsc_v[2], Larow2[dp_y] + tsc_v[2]),
				   ESL_MAX(Jarow1[dp_y] + tsc_v[1], Larow1[dp_y] + tsc_v[1]));		
		Lsc_v[d] = ESL_MAX(Lsc_v[d], IMPOSSIBLE);
		Lsc_v[d] = ESL_MAX(Lsc_v[d], ESL_MAX(Jarow0[dp_y] + tsc_v[0], Larow0[dp_y] + tsc_v[0]));		
	      } /* end of for(d = dn; d <= dx; d++) */
	      break;
	    ...
	    case 6: 
	      ...
	      dp_y = dn - sd; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      for (d = dn; d <= dx; d++) {
		Jsc_v[d] = ESL_MAX(Jarow5[dp_y] + tsc_v[5],
				   init_scAA[v][dp_y]);
		Jsc_v[d] = ESL_MAX(Jsc_v[d], Jarow4[dp_y] + tsc_v[4]);		
		Jsc_v[d] = ESL_MAX(Jsc_v[d], Jarow3[dp_y] + tsc_v[3]);		
		Jsc_v[d] = ESL_MAX(Jsc_v[d], Jarow2[dp_y] + tsc_v[2]);		
		Jsc_v[d] = ESL_MAX(Jsc_v[d], Jarow1[dp_y] + tsc_v[1]);		
		Jsc_v[d] = ESL_MAX(Jsc_v[d], Jarow0[dp_y] + tsc_v[0]);		

		dp_y++;

		Rsc_v[d] = ESL_MAX(ESL_MAX(Jarow5[dp_y] + tsc_v[5], Rarow5[dp_y] + tsc_v[5]),
				   IMPOSSIBLE);
		Rsc_v[d] = ESL_MAX(Rsc_v[d], ESL_MAX(Jarow4[dp_y] + tsc_v[4], Rarow4[dp_y] + tsc_v[4]));		
		Rsc_v[d] = ESL_MAX(Rsc_v[d], ESL_MAX(Jarow3[dp_y] + tsc_v[3], Rarow3[dp_y] + tsc_v[3]));		
		Rsc_v[d] = ESL_MAX(Rsc_v[d], ESL_MAX(Jarow2[dp_y] + tsc_v[2], Rarow2[dp_y] + tsc_v[2]));		
		Rsc_v[d] = ESL_MAX(Rsc_v[d], ESL_MAX(Jarow1[dp_y] + tsc_v[1], Rarow1[dp_y] + tsc_v[1]));		
		Rsc_v[d] = ESL_MAX(Rsc_v[d], ESL_MAX(Jarow0[dp_y] + tsc_v[0], Rarow0[dp_y] + tsc_v[0]));		
	      } /* end of for(d = dn; d <= dx; d++) */
	      ...
	      dp_y = dn - 1; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      for (d = dn; d <= dx; d++, dp_y++) {
		Lsc_v[d] = ESL_MAX(ESL_MAX(Jarow5[dp_y] + tsc_v[5], Larow5[dp_y] + tsc_v[5]),
				   IMPOSSIBLE);
		Lsc_v[d] = ESL_MAX(Lsc_v[d], ESL_MAX(Jarow4[dp_y] + tsc_v[4], Larow4[dp_y] + tsc_v[4]));		
		Lsc_v[d] = ESL_MAX(Lsc_v[d], ESL_MAX(Jarow3[dp_y] + tsc_v[3], Larow3[dp_y] + tsc_v[3]));		
		Lsc_v[d] = ESL_MAX(Lsc_v[d], ESL_MAX(Jarow2[dp_y] + tsc_v[2], Larow2[dp_y] + tsc_v[2]));		
		Lsc_v[d] = ESL_MAX(Lsc_v[d], ESL_MAX(Jarow1[dp_y] + tsc_v[1], Larow1[dp_y] + tsc_v[1]));		
		Lsc_v[d] = ESL_MAX(Lsc_v[d], ESL_MAX(Jarow0[dp_y] + tsc_v[0], Larow0[dp_y] + tsc_v[0]));		
	      } /* end of for(d = dn; d <= dx; d++) */
	      break;
	    ...
	    case 4: 
	      ...
	      dp_y = dn - sd; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      for (d = dn; d <= dx; d++) {
		Jsc_v[d] = ESL_MAX(Jarow3[dp_y] + tsc_v[3],
				   Jarow2[dp_y] + tsc_v[2]);
		Jsc_v[d] = ESL_MAX(Jsc_v[d], init_scAA[v][dp_y]);
		Jsc_v[d] = ESL_MAX(Jsc_v[d], Jarow1[dp_y] + tsc_v[1]);		
		Jsc_v[d] = ESL_MAX(Jsc_v[d], Jarow0[dp_y] + tsc_v[0]);		
		...
		Rsc_v[d] = ESL_MAX(ESL_MAX(Jarow3[dp_y] + tsc_v[3], Rarow3[dp_y] + tsc_v[3]),
				   ESL_MAX(Jarow2[dp_y] + tsc_v[2], Rarow2[dp_y] + tsc_v[2]));
		Rsc_v[d] = ESL_MAX(Rsc_v[d], IMPOSSIBLE);
		Rsc_v[d] = ESL_MAX(Rsc_v[d], ESL_MAX(Jarow1[dp_y] + tsc_v[1], Rarow1[dp_y] + tsc_v[1]));		
		Rsc_v[d] = ESL_MAX(Rsc_v[d], ESL_MAX(Jarow0[dp_y] + tsc_v[0], Rarow0[dp_y] + tsc_v[0]));		
	      ...
	      dp_y = dn - 1; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      for (d = dn; d <= dx; d++, dp_y++) {
		Lsc_v[d] = ESL_MAX(ESL_MAX(Jarow3[dp_y] + tsc_v[3], Larow3[dp_y] + tsc_v[3]),
				   ESL_MAX(Jarow2[dp_y] + tsc_v[2], Larow2[dp_y] + tsc_v[2]));
		Lsc_v[d] = ESL_MAX(Lsc_v[d], IMPOSSIBLE);
		Lsc_v[d] = ESL_MAX(Lsc_v[d], ESL_MAX(Jarow1[dp_y] + tsc_v[1], Larow1[dp_y] + tsc_v[1]));		
		Lsc_v[d] = ESL_MAX(Lsc_v[d], ESL_MAX(Jarow0[dp_y] + tsc_v[0], Larow0[dp_y] + tsc_v[0]));		
	    ...
	    case 5: 
	      ...
	      dp_y = dn - sd; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      for (d = dn; d <= dx; d++) {
		Jsc_v[d] = ESL_MAX(Jarow4[dp_y] + tsc_v[4],
				   Jarow3[dp_y] + tsc_v[3]);
		Jsc_v[d] = ESL_MAX(Jsc_v[d], init_scAA[v][dp_y]);
		Jsc_v[d] = ESL_MAX(Jsc_v[d], Jarow1[dp_y] + tsc_v[1]);		
		Jsc_v[d] = ESL_MAX(Jsc_v[d], Jarow2[dp_y] + tsc_v[2]);		
		...
		Rsc_v[d] = ESL_MAX(ESL_MAX(Jarow4[dp_y] + tsc_v[4], Rarow4[dp_y] + tsc_v[4]),
				   ESL_MAX(Jarow3[dp_y] + tsc_v[3], Rarow3[dp_y] + tsc_v[3]));
		Rsc_v[d] = ESL_MAX(Rsc_v[d], IMPOSSIBLE);
		Rsc_v[d] = ESL_MAX(Rsc_v[d], ESL_MAX(Jarow1[dp_y] + tsc_v[1], Rarow1[dp_y] + tsc_v[1]));		
		Rsc_v[d] = ESL_MAX(Rsc_v[d], ESL_MAX(Jarow2[dp_y] + tsc_v[2], Rarow2[dp_y] + tsc_v[2]));		
		Rsc_v[d] = ESL_MAX(Rsc_v[d], ESL_MAX(Jarow0[dp_y] + tsc_v[0], Rarow0[dp_y] + tsc_v[0]));		
	      ...
	      dp_y = dn - 1; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      for (d = dn; d <= dx; d++, dp_y++) {
		Lsc_v[d] = ESL_MAX(ESL_MAX(Jarow4[dp_y] + tsc_v[4], Larow4[dp_y] + tsc_v[4]),
				   ESL_MAX(Jarow3[dp_y] + tsc_v[3], Larow3[dp_y] + tsc_v[3]));
		Lsc_v[d] = ESL_MAX(Lsc_v[d], IMPOSSIBLE);
		Lsc_v[d] = ESL_MAX(Lsc_v[d], ESL_MAX(Jarow1[dp_y] + tsc_v[1], Larow1[dp_y] + tsc_v[1]));		
		Lsc_v[d] = ESL_MAX(Lsc_v[d], ESL_MAX(Jarow2[dp_y] + tsc_v[2], Larow2[dp_y] + tsc_v[2]));		
	    ...
	    case 2: 
	      ...
	      dp_y = dn - sd; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      for (d = dn; d <= dx; d++) {
		Jsc_v[d] = ESL_MAX(Jarow1[dp_y] + tsc_v[1],
				   init_scAA[v][dp_y]);
		Jsc_v[d] = ESL_MAX(Jsc_v[d], Jarow0[dp_y] + tsc_v[0]);		

		dp_y++;

		Rsc_v[d] = ESL_MAX(ESL_MAX(Jarow1[dp_y] + tsc_v[4], Rarow1[dp_y] + tsc_v[4]),
				   IMPOSSIBLE);
		Rsc_v[d] = ESL_MAX(Rsc_v[d], ESL_MAX(Jarow0[dp_y] + tsc_v[0], Rarow0[dp_y] + tsc_v[0]));
	      }
	      ...
	      dp_y = dn - 1; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      for (d = dn; d <= dx; d++, dp_y++) {
		Lsc_v[d] = ESL_MAX(ESL_MAX(Jarow1[dp_y] + tsc_v[4], Larow1[dp_y] + tsc_v[4]),
				   IMPOSSIBLE);
		Lsc_v[d] = ESL_MAX(Lsc_v[d], ESL_MAX(Jarow0[dp_y] + tsc_v[0], Larow0[dp_y] + tsc_v[0]));
	      } /* end of for(d = dn; d <= dx; d++) */
	      break;
	    } /* end of switch(cnum) */
	    /* add in emission score, and set alpha[jp_v][v][d] cell */
	    assert(dn == 1); 
	    /* handle case where d < 2 */
	    d = dn;
	    i = j - dnA[v] + 1;
	    Jalpha[jp_v][v][d] = IMPOSSIBLE;
	    Lalpha[jp_v][v][d] = lmesc_v[dsq[i]];
	    Ralpha[jp_v][v][d] = rmesc_j;
	    i--;
	    dn++;
	    for (d = dn; d <= dx; d++) {
	      Jalpha[jp_v][v][d] = Jsc_v[d] + esc_v[dsq[i]*cm->abc->Kp+dsq[j]];
	      Lalpha[jp_v][v][d] = Lsc_v[d] + lmesc_v[dsq[i]];
	      Ralpha[jp_v][v][d] = Rsc_v[d] + rmesc_j;
	      i--;
	    }
	  } /* end of else (cm->sttype[v] != MP_st) */
	  else { /* this else is entered if cm->sttype is !(B_st | BEGL_st | ML_st | IL_st | MR_st | IR_st | MP_st) */
	    y    = cm->cfirst[v];
	    float const *Jarow0;
	    float const *Jarow1;
	    float const *Jarow2;
	    ...
	    case 3: 
	      ...
	      dp_y = dn - sd; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      for (d = dn; d <= dx; d++, dp_y++) {
		Jsc = ESL_MAX(Jarow2[dp_y] + tsc_v[2], Jarow1[dp_y] + tsc_v[1]);		
		Jsc = ESL_MAX(Jsc, init_scAA[v][dp_y]);
		Jsc = ESL_MAX(Jsc, Jarow0[dp_y] + tsc_v[0]);		

		Lsc = ESL_MAX(Larow2[dp_y] + tsc_v[2], Larow1[dp_y] + tsc_v[1]);		
		Lsc = ESL_MAX(Lsc, IMPOSSIBLE);
		Lsc = ESL_MAX(Lsc, Larow0[dp_y] + tsc_v[0]);		

		Rsc = ESL_MAX(Rarow2[dp_y] + tsc_v[2], Rarow1[dp_y] + tsc_v[1]);		
		Rsc = ESL_MAX(Rsc, IMPOSSIBLE);
		Rsc = ESL_MAX(Rsc, Rarow0[dp_y] + tsc_v[0]);		
	    ...
	    case 6: 
	      ...
	      dp_y = dn - sd; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      for (d = dn; d <= dx; d++, dp_y++) {
		Jsc = ESL_MAX(Jarow5[dp_y] + tsc_v[5], init_scAA[v][dp_y]);
		Jsc = ESL_MAX(Jsc, Jarow4[dp_y] + tsc_v[4]);		
		Jsc = ESL_MAX(Jsc, Jarow3[dp_y] + tsc_v[3]);		
		Jsc = ESL_MAX(Jsc, Jarow2[dp_y] + tsc_v[2]);		
		Jsc = ESL_MAX(Jsc, Jarow1[dp_y] + tsc_v[1]);		
		...
		Lsc = ESL_MAX(Larow5[dp_y] + tsc_v[5], IMPOSSIBLE);
		Lsc = ESL_MAX(Lsc, Larow4[dp_y] + tsc_v[4]);		
		Lsc = ESL_MAX(Lsc, Larow3[dp_y] + tsc_v[3]);		
		Lsc = ESL_MAX(Lsc, Larow2[dp_y] + tsc_v[2]);		
		Lsc = ESL_MAX(Lsc, Larow1[dp_y] + tsc_v[1]);		
		...
		Rsc = ESL_MAX(Rarow5[dp_y] + tsc_v[5], IMPOSSIBLE);
		Rsc = ESL_MAX(Rsc, Rarow4[dp_y] + tsc_v[4]);		
		Rsc = ESL_MAX(Rsc, Rarow3[dp_y] + tsc_v[3]);		
		Rsc = ESL_MAX(Rsc, Rarow2[dp_y] + tsc_v[2]);		
		Rsc = ESL_MAX(Rsc, Rarow1[dp_y] + tsc_v[1]);		
	    ...
	    case 4: 
	      ...
	      dp_y = dn - sd; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      for (d = dn; d <= dx; d++, dp_y++) {
		Jsc = ESL_MAX(Jarow3[dp_y] + tsc_v[3], Jarow2[dp_y] + tsc_v[2]);
		Jsc = ESL_MAX(Jsc, init_scAA[v][dp_y]);
		Jsc = ESL_MAX(Jsc, Jarow1[dp_y] + tsc_v[1]);		
		Jsc = ESL_MAX(Jsc, Jarow0[dp_y] + tsc_v[0]);		

		Lsc = ESL_MAX(Larow3[dp_y] + tsc_v[3], Larow2[dp_y] + tsc_v[2]);
		Lsc = ESL_MAX(Lsc, IMPOSSIBLE);
		Lsc = ESL_MAX(Lsc, Larow1[dp_y] + tsc_v[1]);		
		Lsc = ESL_MAX(Lsc, Larow0[dp_y] + tsc_v[0]);		

		Rsc = ESL_MAX(Rarow3[dp_y] + tsc_v[3], Rarow2[dp_y] + tsc_v[2]);
		Rsc = ESL_MAX(Rsc, IMPOSSIBLE);
		Rsc = ESL_MAX(Rsc, Rarow1[dp_y] + tsc_v[1]);		
		Rsc = ESL_MAX(Rsc, Rarow0[dp_y] + tsc_v[0]);		
	    ...
	    case 5: 
	      ...
	      dp_y = dn - sd; /* initial dp_y, we increment it at end of 'for(d = ...' loop */
	      for (d = dn; d <= dx; d++, dp_y++) {
		Jsc = ESL_MAX(Jarow4[dp_y] + tsc_v[4], Jarow3[dp_y] + tsc_v[3]);
		Jsc = ESL_MAX(Jsc, init_scAA[v][dp_y]);
		Jsc = ESL_MAX(Jsc, Jarow1[dp_y] + tsc_v[1]);		
		Jsc = ESL_MAX(Jsc, Jarow2[dp_y] + tsc_v[2]);		
		Jsc = ESL_MAX(Jsc, Jarow0[dp_y] + tsc_v[0]);		

		Lsc = ESL_MAX(Larow4[dp_y] + tsc_v[4], Larow3[dp_y] + tsc_v[3]);
		Lsc = ESL_MAX(Lsc, IMPOSSIBLE);
		Lsc = ESL_MAX(Lsc, Larow1[dp_y] + tsc_v[1]);		
		Lsc = ESL_MAX(Lsc, Larow2[dp_y] + tsc_v[2]);		
		Lsc = ESL_MAX(Lsc, Larow0[dp_y] + tsc_v[0]);		

		Rsc = ESL_MAX(Rarow4[dp_y] + tsc_v[4], Rarow3[dp_y] + tsc_v[3]);
		Rsc = ESL_MAX(Rsc, IMPOSSIBLE);
		Rsc = ESL_MAX(Rsc, Rarow1[dp_y] + tsc_v[1]);		
		Rsc = ESL_MAX(Rsc, Rarow2[dp_y] + tsc_v[2]);		
		Rsc = ESL_MAX(Rsc, Rarow0[dp_y] + tsc_v[0]);		
		...
		Lsc = ESL_MAX(Larow1[dp_y] + tsc_v[1], IMPOSSIBLE);
		Lsc = ESL_MAX(Lsc, Larow0[dp_y] + tsc_v[0]);		

		Rsc = ESL_MAX(Rarow1[dp_y] + tsc_v[1], IMPOSSIBLE);
		Rsc = ESL_MAX(Rsc, Rarow0[dp_y] + tsc_v[0]);		
	  ...
	  } /* end of else (sttype != (BEGL_S | B_st | ML_st | IL_st | MR_st | IR_st | MP_st */
	  if(vsc != NULL) {
	    if(cm->stid[v] == BIF_B) { 
	      for (d = dnA[v]; d <= dxA[v]; d++) { 
		vsc[v] = ESL_MAX(vsc[v], 
	  ...
	  if(cm->stid[v] == BIF_B) { 
	    for(d = dnA[v]; d <= dxA[v]; d++) { 
	      printf("F j: %3d  v: %3d  d: %3d\n", j, v, d);
	      printf("F j: %3d  v: %3d  d: %3d   J: %10.4f  L: %10.4f  R: %10.4f  T: %10.4f\n", 
		     j, v, d, 
  ...
  for (i = 0; i < N; i++)
    {
      ...
      esl_stopwatch_Start(w);
      if((status = FastTrCYKScan(cm, errbuf, trsmx, dsq, 1, L, 0., NULL, FALSE, 0., NULL, NULL, NULL, &sc)) != eslOK) cm_Fail(errbuf);
      printf("%4d %-30s %10.4f bits ", (i+1), "FastTrCYKScan(): ", sc);
      esl_stopwatch_Stop(w);
      esl_stopwatch_Display(stdout, w, " CPU time: ");
