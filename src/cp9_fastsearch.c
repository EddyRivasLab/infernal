 ...
 */
float
cp9_EXPTLFastViterbi(CM_t *cm, const CP9_OPROFILE *om, const CP9_OMX *ox, ESL_DSQ *dsq, 
...
main(int argc, char **argv)
{
  ...
  for (i = 0; i < N; i++)
	...
	{ 
	  esl_stopwatch_Start(w);
	  sc = cp9_EXPTLFastViterbi(cm, om, ox, dsq, 1, L, cm->W, 0., NULL, NULL, NULL,
				    TRUE,   /* we're scanning */
				    FALSE,  /* we're not ultimately aligning */
				    (! esl_opt_GetBoolean(go, "-f")),  /* memory efficient ? */
				    NULL,   /* don't want the DP matrix back */
				    NULL);  /* don't want traces back */
	  printf("%4d %-30s %10.4f bits ", (i+1), "cp9_EXPTLFastViterbi(): ", sc);
	  esl_stopwatch_Stop(w);
	  esl_stopwatch_Display(stdout, w, " CPU time: ");
	}
    }
