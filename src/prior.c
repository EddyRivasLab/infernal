...
static struct p7prior_s *default_infernal_nucleic_prior(void);
struct p7prior_s *P7DefaultInfernalPrior(void);
...
/* EPN 10.10.05 
 * Use the single emission prior from Infernal 0.56 for the P7 emission
 * prior. From hmmer's prior.c's P7DefaultPrior()
 *
 * Function: P7DefaultInfernalPrior()
 * 
 * Purpose:  Set up a somewhat more realistic single component
 *           Dirichlet prior than Laplace.
 */ 
struct p7prior_s *
P7DefaultInfernalPrior(void)
{
  switch (Alphabet_type) {
  case hmmAMINO:     Die("Shouldn't be hmmAMINO");
  case hmmNUCLEIC:   return default_infernal_nucleic_prior();
...
/* Function: default_infernal_nucleic_prior()
 * 
 * Purpose:  Set the default DNA prior. Based on HMMER transitions
 *           and Infernal's singlet emission Dirichlet mixture.
 */
static struct p7prior_s *
default_infernal_nucleic_prior(void)
{
  ...
  /*  EPN for tracing sake, if you want to use a plus 1 prior,
      then uncomment the following block for plus-1 transitions. */
  /*
  pri->tnum        = 1;
  pri->tq[0]       = 1.;
  pri->t[0][TMM]   = 1.;
  ...
  */
  ...
  /* the HMMER 2.4 default transition priors */
  pri->tnum        = 1;
  pri->tq[0]       = 1.;
  pri->t[0][TMM]   = 0.7939;
  pri->t[0][TMI]   = 0.0278;
  ...
  /* EPN 10.10.05 Now set the emissions (hard-coded for now)
     These are set to the singlet emission priors from Infernal 0.56,
     really we should reestimate priors from a dataset where we
     use ALL columns from RNA alignments (SSU is what we normally use).
     These priors were estimated from counts derived from columns that
     were explicitly labelled as single-stranded, making them 'appropriate'
     for Infernal singlet emissions, but not for HMM emissions.
  */
     
  pri->mnum = 8;
  pri->mq[0] = 0.085091850427;
  pri->m[0][0] = 0.575686380127;
  pri->m[0][1] = 0.756214632926;
  pri->m[0][2] = 0.340269621276;
  pri->m[0][3] = 13.774558068728;
  
  pri->mq[1] = 0.015935406086;
  pri->m[1][0] = 153.865583955384;
  pri->m[1][1] = 0.235000107300;
  pri->m[1][2] = 0.356622653787;
  pri->m[1][3] = 0.006812718667;

  pri->mq[2] = 0.102013232739;
  pri->m[2][0] = 176.440373997567;
  pri->m[2][1] = 0.935905951648;
  pri->m[2][2] = 1.292808081312;
  pri->m[2][3] = 1.617069444109;
  
  pri->mq[3] = 0.415954530541;
  pri->m[3][0] = 1.696250324914;
  pri->m[3][1] = 1.128033754503;
  pri->m[3][2] = 0.955462899400;
  pri->m[3][3] = 1.676465850057;
  
  pri->mq[4] = 0.074470557341;
  pri->m[4][0] = 0.074365531036;
  pri->m[4][1] = 0.039185613484;
  pri->m[4][2] = 0.063868972113;
  pri->m[4][3] = 0.042432587902;
  
  pri->mq[5] = 0.055442639402;
  pri->m[5][0] = 0.615068901818;
  pri->m[5][1] = 14.630712353118;
  pri->m[5][2] = 0.298404817403;
  pri->m[5][3] = 0.864718655041;
  
  pri->mq[6] = 0.118379098369;
  pri->m[6][0] = 1.163176461349;
  pri->m[6][1] = 0.408090165233;
  pri->m[6][2] = 11.188793743319;
  pri->m[6][3] = 0.699118301558;
  
  pri->mq[7] = 0.132712685095;
  pri->m[7][0] = 16.417200192194;
  pri->m[7][1] = 0.980503286582;
  pri->m[7][2] = 1.132071515554;
  pri->m[7][3] = 1.376129445524;

  /*  EPN for tracing sake, if you want to use a plus 1 prior,
      then uncomment the following block for plus-1 emissions */
  /*
   pri->mnum  = 1;
   pri->mq[0] = 1.;
  ...
  */
   ...
   /* insertions are plus-1 for HMMER 2.4 and stay plus-1 here */
   pri->inum  = 1;
   pri->iq[0] = 1.;
