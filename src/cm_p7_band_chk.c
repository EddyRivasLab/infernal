 ...
 * scheme holding O(sqrt(L)*avg_bw) working memory, producing BYTE-IDENTICAL
 * cp9b bands vs the non-checkpointed int path.
 *
 * Design (see project memory project_ckpt_dual_sweep):
 *   cp9_FB2HMMBandsP7B derives per-NODE bands via TWO opposite-direction
 *   cumulative ILogsum sweeps (MIN ascending i; MAX descending i). A single
 *   HMMER-style descending decode does not map. So:
 ...
 *   Byte-identity: the per-row recurrences below are verbatim copies of the
 *   loop bodies in cp9_ForwardP7B_OLD_WITH_EL / cp9_BackwardP7B / the posterior
 *   in cp9_FB2HMMBandsP7B (cm_p7_band.c), with mmx[i]->mc, mmx[i-1]->mp,
 *   mmx[i+1]->mn substitutions. The originals are kept untouched as the
 *   byte-diff reference.
 */
...
/*****************************************************************
 * 1. Per-row recurrence kernels (verbatim copies of the originals).
 *****************************************************************/

/* Forward row 0 (init). Verbatim from cp9_ForwardP7B_OLD_WITH_EL i=0 block.
 * Planes mc/ic/dc/ec are row-0 (offset 0 = kmin[0]==0). */
static void
cp9_chk_fwd_row0(CP9_t *cp9, int *kmin, int *kmax, int M,
                 int *mc, int *ic, int *dc, int *ec, int *ret_erow0)
{
  ...
  for (k = kn; k <= kmax[0]; k++, kpcur++) {
    mc[kpcur] = ic[kpcur] = ec[kpcur] = -INFTY;
    sc = ILogsum(ILogsum(mc[kpcur-1] + CP9TSC(cp9O_MD,k-1),
                         ic[kpcur-1] + CP9TSC(cp9O_ID,k-1)),
                 dc[kpcur-1] + CP9TSC(cp9O_DD,k-1));
    dc[kpcur] = sc;
  }
  erow0 = -INFTY;
  if(INBAND(0, M)) { erow0 = dc[M] + CP9TSC(cp9O_DM,M); } /* kmin[0]==0 so abs M == rel M */
  if(ret_erow0) *ret_erow0 = erow0;
}

/* Forward row i (1..L). Verbatim from cp9_ForwardP7B_OLD_WITH_EL main loop body.
 * Cur planes mc/ic/dc/ec (offset 0 = kmin[i]); prev planes mp/ip/dp/ep
 * (offset 0 = kmin[i-1]). */
static void
cp9_chk_fwd_row(CP9_t *cp9, ESL_DSQ *dsq, int i, int *kmin, int *kmax, int M,
                int *mp, int *ip, int *dp, int *ep,
                int *mc, int *ic, int *dc, int *ec, int *ret_erow)
{
  int const *tsc = cp9->otsc;
  ...
  int sc, k, c, kn, kx, kpcur, kpprv, kpprv_el;

  if(kmin[i] == 0) {
    mc[0]  = -INFTY;
    dc[0]  = -INFTY;
    ec[0]  = -INFTY;
    sc = ILogsum(ILogsum(mp[0] + CP9TSC(cp9O_MI,0),
                         ip[0] + CP9TSC(cp9O_II,0)),
                 dp[0] + CP9TSC(cp9O_DI,0));
    ic[0] = sc + isc[0];
    kn = 1;
  }
  else {
  ...
  for (kpcur = 0; kpcur < ESL_MIN(kn-kmin[i], kmax[i]-kmin[i]+1); kpcur++) mc[kpcur] = -INFTY;
  for (kpcur = ESL_MAX(0,kx-kmin[i]+1); kpcur <= kmax[i]-kmin[i]; kpcur++) mc[kpcur] = -INFTY;
  for (kpcur = 0; kpcur < ESL_MIN(kn-kmin[i], kmax[i]-kmin[i]+1); kpcur++) ec[kpcur] = -INFTY;
  for (kpcur = ESL_MAX(0,kx-kmin[i]+1); kpcur <= kmax[i]-kmin[i]; kpcur++) ec[kpcur] = -INFTY;

  kpcur = kn - kmin[i];
  kpprv = kn - kmin[i-1];
  for (k = kn; k <= kx; k++, kpcur++, kpprv++) {
    sc = ILogsum(ILogsum(mp[kpprv-1] + CP9TSC(cp9O_MM,k-1),
                         ip[kpprv-1] + CP9TSC(cp9O_IM,k-1)),
                 dp[kpprv-1] + CP9TSC(cp9O_DM,k-1));
    if(INBAND(i-1, 0)) {
      assert(kmin[(i-1)] == 0);
      if(mp[0] != -INFTY)
        sc = ILogsum(sc, mp[0] + CP9TSC(cp9O_BM,k));
    }
    for(c = 0; c < cp9->el_from_ct[k]; c++) {
      if(INBAND(i-1, cp9->el_from_idx[k][c])) {
        kpprv_el = cp9->el_from_idx[k][c] - kmin[(i-1)];
        sc = ILogsum(sc, ep[kpprv_el]);
      }
    }
    if(sc != -INFTY) {
      mc[kpcur] = sc + msc[k];
      endsc = ILogsum(endsc, mc[kpcur] + CP9TSC(cp9O_ME,k));
    }
    else {
      mc[kpcur] = -INFTY;
    }
    ...
    if((cp9->flags & CPLAN9_EL) && cp9->has_el[k]) {
      sc = mc[kpcur] + CP9TSC(cp9O_MEL,k);
      if(INBAND(i-1, k)) {
        kpprv_el = k - kmin[(i-1)];
        sc = ILogsum(sc, ep[kpprv_el] + el_selfsc);
      }
    }
    ec[kpcur] = sc;
  }
  ...
  for (kpcur = 0; kpcur < ESL_MIN(kn-kmin[i], kmax[i]-kmin[i]+1); kpcur++) ic[kpcur] = -INFTY;
  for (kpcur = ESL_MAX(0,kx-kmin[i]+1); kpcur <= kmax[i]-kmin[i]; kpcur++) ic[kpcur] = -INFTY;
  kpcur = kn - kmin[i];
  kpprv = kn - kmin[i-1];
  for (k = kn; k <= kx; k++, kpcur++, kpprv++) {
    sc = ILogsum(ILogsum(mp[kpprv] + CP9TSC(cp9O_MI,k),
                         ip[kpprv] + CP9TSC(cp9O_II,k)),
                 dp[kpprv] + CP9TSC(cp9O_DI,k));
    if(sc != -INFTY) ic[kpcur] = sc + isc[k];
    else             ic[kpcur] = -INFTY;
  }

  kn = kmin[i]+1;
  for (kpcur = 0; kpcur < (kn - kmin[i]); kpcur++) dc[kpcur] = -INFTY;
  kpcur = kn - kmin[i];
  ...
  if(INBAND(i, M)) {
    endsc = ILogsum(ILogsum(endsc, dc[M-kmin[i]] + CP9TSC(cp9O_DM,M)),
                    ic[M-kmin[i]] + CP9TSC(cp9O_IM,M));
    for(c = 0; c < cp9->el_from_ct[M+1]; c++) {
      if(INBAND(i, cp9->el_from_idx[M+1][c])) {
        kpprv_el = cp9->el_from_idx[M+1][c] - kmin[i];
        endsc = ILogsum(endsc, ec[kpprv_el]);
      }
    }
  }
...
/* Backward row L (init). Verbatim from cp9_BackwardP7B i=L block.
 * Cur planes mc/ic/dc/ec for row L. */
static void
cp9_chk_bwd_rowL(CP9_t *cp9, ESL_DSQ *dsq, int L, int *kmin, int *kmax, int M,
                 int *mc, int *ic, int *dc, int *ec)
{
  int const *tsc = cp9->otsc;
  int i = L;
  ...
  kpcur = 0;
  for (k = kmin[i]; k <= kmax[i]; k++, kpcur++) ec[kpcur] = -INFTY;
  ...
  if(INBAND(i, M)) {
    ...
    kpcur = M-kmin[i];
    mc[kpcur]  = 0. +
      ILogsum(ec[kpcur] + CP9TSC(cp9O_MEL, M),
              CP9TSC(cp9O_ME,M));
    mc[kpcur] += cp9->msc[dsq[i]][M];
    ic[kpcur]  = 0. + CP9TSC(cp9O_IM,M);
    ic[kpcur] += cp9->isc[dsq[i]][M];
    dc[kpcur]  = CP9TSC(cp9O_DM,M);
    kx = M-1;
    kpcur--;
  }
  ...
  for (k = kx; k >= kmin[i]; k--, kpcur--)
    {
      mc[kpcur]  = 0 + CP9TSC(cp9O_ME,k);
      if(INBAND(i, k+1)) {
        mc[kpcur]  = ILogsum(mc[kpcur], dc[kpcur+1] + CP9TSC(cp9O_MD,k));
      }
      if(cp9->flags & CPLAN9_EL)
        mc[kpcur]  = ILogsum(mc[kpcur], ec[kpcur] + CP9TSC(cp9O_MEL,k));
      mc[kpcur] += cp9->msc[dsq[i]][k];

      if(INBAND(i, k+1)) {
        ic[kpcur]  = dc[kpcur+1] + CP9TSC(cp9O_ID,k);
        ic[kpcur] += cp9->isc[dsq[i]][k];
        dc[kpcur]  = dc[kpcur+1] + CP9TSC(cp9O_DD,k);
      }
      else {
        ic[kpcur] = -INFTY;
        dc[kpcur] = -INFTY;
  ...
  if(INBAND(i, 0)) {
    mc[0]  = dc[1] + CP9TSC(cp9O_MD,0);
    ic[0]  = dc[1] + CP9TSC(cp9O_ID,0);
    ic[0] += cp9->isc[dsq[i]][0];
    dc[0]   = -INFTY;
    ec[0]  = -INFTY;
  }
}

/* Backward row i (1..L-1). Verbatim from cp9_BackwardP7B main loop body.
 * Cur planes mc/ic/dc/ec (row i); next planes mn/in/dn/en (row i+1). */
static void
cp9_chk_bwd_row(CP9_t *cp9, ESL_DSQ *dsq, int i, int *kmin, int *kmax, int M,
                int *mc, int *ic, int *dc, int *ec,
                int *mn, int *in, int *dn, int *en)
{
  int const *tsc = cp9->otsc;
  ...
  if(INBAND(i, M)) {
    ...
    if((cp9->flags & CPLAN9_EL) && (cp9->has_el[M]))
      ec[kpcur] = ec[kpcur] + cp9->el_selfsc;

    if(INBAND(i+1, M)) {
      kpprv = M-kmin[i+1];
      mc[kpcur]  = in[kpprv] + CP9TSC(cp9O_MI,M);
      mc[kpcur] += cp9->msc[dsq[i]][M];
      ic[kpcur]  = in[kpprv] + CP9TSC(cp9O_II,M);
      ic[kpcur] += cp9->isc[dsq[i]][M];
      dc[kpcur]  = in[kpprv] + CP9TSC(cp9O_DI,M);
    }
    else {
      mc[kpcur] = ic[kpcur] = dc[kpcur] = -INFTY;
    }

    if((cp9->flags & CPLAN9_EL) && (cp9->has_el[M]))
      mc[kpcur] = ILogsum(mc[kpcur], ec[kpcur] + CP9TSC(cp9O_MEL,M));
      ...
      if(cp9->flags & CPLAN9_EL) {
        for(c = 0; c < cp9->el_from_ct[M]; c++)
          if(INBAND(i, cp9->el_from_idx[M][c])) {
            ...
            ec[kpcur_el] = ILogsum(ec[kpcur_el], mn[kpprv]);
          }
      }
    }
  }
  ...
  for (kpcur = 0; kpcur < ESL_MIN(kn-kmin[i], kmax[i]-kmin[i]+1); kpcur++) mc[kpcur] = ic[kpcur] = dc[kpcur] = ec[kpcur] = -INFTY;
  for (kpcur = ESL_MAX(0,kx-kmin[i]+1); kpcur <= kmax[i]-kmin[i]; kpcur++) mc[kpcur] = ic[kpcur] = dc[kpcur] = ec[kpcur] = -INFTY;
  ...
  for (k = kx; k >= kn; k--, kpcur--, kpprv--)
    {
      ...
      if(INBAND(i+1, k)) {
        if((cp9->flags & CPLAN9_EL) && (cp9->has_el[k]))
          ec[kpcur] = ILogsum(ec[kpcur], en[kpprv] + cp9->el_selfsc);
      }
      mc[kpcur] = mn[kpprv+1] + CP9TSC(cp9O_MM,k);
      ic[kpcur] = mn[kpprv+1] + CP9TSC(cp9O_IM,k);
      dc[kpcur] = mn[kpprv+1] + CP9TSC(cp9O_DM,k);
    }
  ...
  for (k = kx; k >= kn; k--, kpcur--, kpprv--)
    {
      mc[kpcur] = ILogsum(mc[kpcur], in[kpprv] + CP9TSC(cp9O_MI,k));
      ic[kpcur] = ILogsum(ic[kpcur], in[kpprv] + CP9TSC(cp9O_II,k));
      dc[kpcur] = ILogsum(dc[kpcur], in[kpprv] + CP9TSC(cp9O_DI,k));
    }
  ...
  for (k = kx; k >= kn; k--, kpcur--)
    {
      mc[kpcur] = ILogsum(mc[kpcur], dc[kpcur+1] + CP9TSC(cp9O_MD,k));
      ic[kpcur] = ILogsum(ic[kpcur], dc[kpcur+1] + CP9TSC(cp9O_ID,k));
      dc[kpcur] = ILogsum(dc[kpcur], dc[kpcur+1] + CP9TSC(cp9O_DD,k));
      mc[kpcur] += cp9->msc[dsq[i]][k];
      ic[kpcur] += cp9->isc[dsq[i]][k];
    }
  ...
  if(INBAND(i, 0)) {
    ...
    if(INBAND(i+1, 1)) {
      if(mn[kpprv+1] != -INFTY)
        ic[kpcur] = ILogsum(ic[kpcur], mn[kpprv+1] + CP9TSC(cp9O_IM,0));
    }
    if(INBAND(i+1, 0)) {
      if(in[kpprv] != -INFTY)
        ic[kpcur] = ILogsum(ic[kpcur], in[kpprv] + CP9TSC(cp9O_II,0));
    }
    if(INBAND(i, 1)) {
      if(dc[kpcur+1] != -INFTY)
        ic[kpcur] = ILogsum(ic[kpcur], dc[kpcur+1] + CP9TSC(cp9O_ID,0));
    }
    ...
    for(kprv = kprvx; kprv >= kprvn; kprv--, kpprv--) {
      if(mn[kpprv] != -INFTY)
        mc[kpcur] = ILogsum(mc[kpcur], (mn[kpprv] + CP9TSC(cp9O_BM,kprv)));
    }
    k = 0;
    if(INBAND(i+1, 0)) {
      kpprv = k - kmin[i+1];
      if(in[kpprv] != -INFTY) {
        mc[kpcur] = ILogsum(mc[kpcur], (in[kpprv] + CP9TSC(cp9O_MI,0)));
      }
    }
    if(INBAND(i, 1)) {
      if(dc[kpcur+1] != -INFTY) {
        mc[kpcur] = ILogsum(mc[kpcur], (dc[kpcur+1] + CP9TSC(cp9O_MD,0)));
      }
    }
  }
}

/* Backward row 0. Verbatim from cp9_BackwardP7B i==0 block.
 * Cur planes mc/ic/dc/ec (row 0); next planes mn/in/dn/en (row 1). */
static void
cp9_chk_bwd_row0(CP9_t *cp9, ESL_DSQ *dsq, int *kmin, int *kmax, int M,
                 int *mc, int *ic, int *dc, int *ec,
                 int *mn, int *in, int *dn, int *en)
{
  int const *tsc = cp9->otsc;
  ...
  for (kpcur = 0; kpcur <= kmax[0] - kmin[0]; kpcur++) mc[kpcur] = ic[kpcur] = dc[kpcur] = ec[kpcur] = -INFTY;
...
/*****************************************************************
 * 2. Checkpoint store + block layout.
 *****************************************************************/
...
typedef struct {
  ...
  int   nbnd;      /* # boundaries */
  int  *bnd;       /* [0..nbnd-1] boundary row indices: 0, B, 2B, ..., L */
  int64_t *off;    /* [0..nbnd-1] cell offset of each boundary row's planes */
  int64_t ncells;  /* total stored cells (sum of band widths at boundaries) */
  /* Forward checkpoint planes */
  int *fmmx, *fimx, *fdmx, *felmx;
  /* Backward checkpoint planes */
  int *bmmx, *bimx, *bdmx, *belmx;
} cp9chk_t;

static void
cp9chk_Destroy(cp9chk_t *s)
{
  if(s == NULL) return;
  if(s->bnd)  free(s->bnd);
  if(s->off)  free(s->off);
...
static cp9chk_t *
cp9chk_Create(int L, int M, int *kmin, int *kmax, char *errbuf)
{
  int status;
  cp9chk_t *s = NULL;
  int blk, cap, j, r;
  ...
  ESL_ALLOC(s->fmmx,  sizeof(int) * s->ncells);
  ESL_ALLOC(s->fimx,  sizeof(int) * s->ncells);
  ESL_ALLOC(s->fdmx,  sizeof(int) * s->ncells);
  ESL_ALLOC(s->felmx, sizeof(int) * s->ncells);
  ESL_ALLOC(s->bmmx,  sizeof(int) * s->ncells);
  ESL_ALLOC(s->bimx,  sizeof(int) * s->ncells);
  ESL_ALLOC(s->bdmx,  sizeof(int) * s->ncells);
  ESL_ALLOC(s->belmx, sizeof(int) * s->ncells);

  return s;

 ERROR:
  cp9chk_Destroy(s);
  if(errbuf) sprintf(errbuf, "cp9chk_Create: OOM");
  return NULL;
}

/* Forward-checkpointed fill: roll 2 rows over 1..L, store F planes at each
 * boundary row. Boundary row 0 stored from cp9_chk_fwd_row0. */
static int
cp9chk_FwdFill(cp9chk_t *s, CP9_t *cp9, ESL_DSQ *dsq, int *kmin, int *kmax, char *errbuf)
{
  int status;
  int M = s->M, L = s->L;
  int *m0,*i0,*d0,*e0, *m1,*i1,*d1,*e1; /* two rolling rows, width M+1 each */
  int *mp,*ip,*dp,*ep, *mc,*ic,*dc,*ec;
  int i, j, w;

  ESL_ALLOC(m0, sizeof(int)*(M+1)); ESL_ALLOC(i0, sizeof(int)*(M+1));
  ESL_ALLOC(d0, sizeof(int)*(M+1)); ESL_ALLOC(e0, sizeof(int)*(M+1));
  ESL_ALLOC(m1, sizeof(int)*(M+1)); ESL_ALLOC(i1, sizeof(int)*(M+1));
  ESL_ALLOC(d1, sizeof(int)*(M+1)); ESL_ALLOC(e1, sizeof(int)*(M+1));

  /* row 0 -> buffer 0. Buffers are zeroed before each kernel call to replicate
   * GrowCP9Matrix's one-time memset-0 (the recurrences read some cells before
   * assigning them; correctness depends on those reading 0). */
  mc=m0; ic=i0; dc=d0; ec=e0;
  memset(mc,0,sizeof(int)*(M+1)); memset(ic,0,sizeof(int)*(M+1)); memset(dc,0,sizeof(int)*(M+1)); memset(ec,0,sizeof(int)*(M+1));
  cp9_chk_fwd_row0(cp9, kmin, kmax, M, mc, ic, dc, ec, NULL);
  j = 0; /* next boundary to store; bnd[0] == 0 */
  if(s->bnd[j] == 0) {
    w = kmax[0]-kmin[0]+1;
    memcpy(s->fmmx + s->off[j], mc, sizeof(int)*w);
    memcpy(s->fimx + s->off[j], ic, sizeof(int)*w);
    memcpy(s->fdmx + s->off[j], dc, sizeof(int)*w);
    memcpy(s->felmx+ s->off[j], ec, sizeof(int)*w);
    j++;
  }
  ...
  for(i = 1; i <= L; i++) {
    ...
    memset(mc,0,sizeof(int)*(M+1)); memset(ic,0,sizeof(int)*(M+1)); memset(dc,0,sizeof(int)*(M+1)); memset(ec,0,sizeof(int)*(M+1));
    cp9_chk_fwd_row(cp9, dsq, i, kmin, kmax, M, mp, ip, dp, ep, mc, ic, dc, ec, NULL);
  ...
  return eslOK;
 ERROR:
  if(errbuf) sprintf(errbuf, "cp9chk_FwdFill: OOM");
  return status;
}

/* Backward-checkpointed fill: roll 2 rows over L..0, store B planes at each
 * boundary row. Returns sc = bmx->mmx[0][0]. */
static int
cp9chk_BwdFill(cp9chk_t *s, CP9_t *cp9, ESL_DSQ *dsq, int *kmin, int *kmax, int *ret_sc, char *errbuf)
{
  int status;
  int M = s->M, L = s->L;
  int *m0,*i0,*d0,*e0, *m1,*i1,*d1,*e1;
  int *mn,*in,*dn,*en, *mc,*ic,*dc,*ec;
  int i, j, w;
  ...
  j = s->nbnd - 1; /* boundary to store; bnd[nbnd-1] == L */

  /* row L. Buffer = i%2 (cur and next=i+1 always differ in parity). */
  if(L & 1) { mc=m1; ic=i1; dc=d1; ec=e1; } else { mc=m0; ic=i0; dc=d0; ec=e0; }
  memset(mc,0,sizeof(int)*(M+1)); memset(ic,0,sizeof(int)*(M+1)); memset(dc,0,sizeof(int)*(M+1)); memset(ec,0,sizeof(int)*(M+1));
  cp9_chk_bwd_rowL(cp9, dsq, L, kmin, kmax, M, mc, ic, dc, ec);
  if(s->bnd[j] == L) {
    w = kmax[L]-kmin[L]+1;
    memcpy(s->bmmx + s->off[j], mc, sizeof(int)*w);
    memcpy(s->bimx + s->off[j], ic, sizeof(int)*w);
    memcpy(s->bdmx + s->off[j], dc, sizeof(int)*w);
    memcpy(s->belmx+ s->off[j], ec, sizeof(int)*w);
    j--;
  }
  mn=mc; in=ic; dn=dc; en=ec; /* next row (L) = whatever buffer row L used */

  for(i = L-1; i >= 1; i--) {
    if(i & 1) { mc=m1; ic=i1; dc=d1; ec=e1; }  /* buffer = i%2; next (i+1) differs */
    else      { mc=m0; ic=i0; dc=d0; ec=e0; }
    memset(mc,0,sizeof(int)*(M+1)); memset(ic,0,sizeof(int)*(M+1)); memset(dc,0,sizeof(int)*(M+1)); memset(ec,0,sizeof(int)*(M+1));
    cp9_chk_bwd_row(cp9, dsq, i, kmin, kmax, M, mc, ic, dc, ec, mn, in, dn, en);
    if(j >= 0 && s->bnd[j] == i) {
      w = kmax[i]-kmin[i]+1;
      memcpy(s->bmmx + s->off[j], mc, sizeof(int)*w);
      memcpy(s->bimx + s->off[j], ic, sizeof(int)*w);
  ...
  /* row 0 (buffer = 0%2 = m0; next=row1 used buffer 1 -> differ) */
  mc=m0; ic=i0; dc=d0; ec=e0;
  memset(mc,0,sizeof(int)*(M+1)); memset(ic,0,sizeof(int)*(M+1)); memset(dc,0,sizeof(int)*(M+1)); memset(ec,0,sizeof(int)*(M+1));
  cp9_chk_bwd_row0(cp9, dsq, kmin, kmax, M, mc, ic, dc, ec, mn, in, dn, en);
  *ret_sc = mc[0]; /* bmx->mmx[0][0] */
  ...
  return eslOK;
 ERROR:
  if(errbuf) sprintf(errbuf, "cp9chk_BwdFill: OOM");
  return status;
}

/*****************************************************************
 * 3. Segment materialize + dual-sweep band reduction.
 *****************************************************************/

/* A reusable segment workspace: holds fwd[a..b] and bck[a..b] planes for one
 * segment, with per-row pointers. Sized to the largest segment. */
typedef struct {
  int     maxrows;  /* blk+1 */
  int64_t maxcells; /* largest segment cell count */
  int    *fm,*fi,*fd,*fe;     /* forward contiguous mem */
  int    *bm,*bi,*bd,*be;     /* backward contiguous mem */
  int   **fmr,**fir,**fdr,**fer; /* per-row ptrs (idx 0..rows-1) */
  int   **bmr,**bir,**bdr,**ber;
  int    *pm,*pi,*pd;         /* one posterior scratch row, width maxw */
} cp9seg_t;

static void
cp9seg_Destroy(cp9seg_t *g)
{
  if(g == NULL) return;
...
static cp9seg_t *
cp9seg_Create(cp9chk_t *s, int *kmin, int *kmax, char *errbuf)
{
  int status;
  cp9seg_t *g = NULL;
  int j, r, maxw = 0;
  ...
  ESL_ALLOC(g->fm, sizeof(int)*maxcells); ESL_ALLOC(g->fi, sizeof(int)*maxcells);
  ESL_ALLOC(g->fd, sizeof(int)*maxcells); ESL_ALLOC(g->fe, sizeof(int)*maxcells);
  ESL_ALLOC(g->bm, sizeof(int)*maxcells); ESL_ALLOC(g->bi, sizeof(int)*maxcells);
  ESL_ALLOC(g->bd, sizeof(int)*maxcells); ESL_ALLOC(g->be, sizeof(int)*maxcells);
  ESL_ALLOC(g->fmr, sizeof(int*)*(g->maxrows+1)); ESL_ALLOC(g->fir, sizeof(int*)*(g->maxrows+1));
  ESL_ALLOC(g->fdr, sizeof(int*)*(g->maxrows+1)); ESL_ALLOC(g->fer, sizeof(int*)*(g->maxrows+1));
  ESL_ALLOC(g->bmr, sizeof(int*)*(g->maxrows+1)); ESL_ALLOC(g->bir, sizeof(int*)*(g->maxrows+1));
  ESL_ALLOC(g->bdr, sizeof(int*)*(g->maxrows+1)); ESL_ALLOC(g->ber, sizeof(int*)*(g->maxrows+1));
  ESL_ALLOC(g->pm, sizeof(int)*maxw); ESL_ALLOC(g->pi, sizeof(int)*maxw); ESL_ALLOC(g->pd, sizeof(int)*maxw);

  return g;
 ERROR:
  cp9seg_Destroy(g);
  if(errbuf) sprintf(errbuf, "cp9seg_Create: OOM");
  return NULL;
}

/* Materialize segment j = rows [a..b]: fwd from F-ckpt at a, bck from B-ckpt at b.
 * Sets per-row pointers g->{f,b}*r[r-a]. */
static void
cp9seg_Fill(cp9seg_t *g, cp9chk_t *s, int j, CP9_t *cp9, ESL_DSQ *dsq, int *kmin, int *kmax)
{
  int M = s->M, L = s->L;
  int a = s->bnd[j-1], b = s->bnd[j];
  int r, w;
  ...
  /* zero buffers to replicate GrowCP9Matrix's memset-0 (recurrences read some
   * cells before assigning them). */
  segcells = 0;
  for(r = a; r <= b; r++) segcells += (kmax[r]-kmin[r]+1);
  memset(g->fm,0,sizeof(int)*segcells); memset(g->fi,0,sizeof(int)*segcells);
  memset(g->fd,0,sizeof(int)*segcells); memset(g->fe,0,sizeof(int)*segcells);
  memset(g->bm,0,sizeof(int)*segcells); memset(g->bi,0,sizeof(int)*segcells);
  memset(g->bd,0,sizeof(int)*segcells); memset(g->be,0,sizeof(int)*segcells);

  /* set row pointers into contiguous mem */
  fo = 0; bo = 0;
  for(r = a; r <= b; r++) {
    w = kmax[r]-kmin[r]+1;
  ...
  w = kmax[a]-kmin[a]+1;
  memcpy(g->fmr[0], s->fmmx + s->off[j-1], sizeof(int)*w);
  memcpy(g->fir[0], s->fimx + s->off[j-1], sizeof(int)*w);
  memcpy(g->fdr[0], s->fdmx + s->off[j-1], sizeof(int)*w);
  memcpy(g->fer[0], s->felmx+ s->off[j-1], sizeof(int)*w);
  for(r = a+1; r <= b; r++) {
    cp9_chk_fwd_row(cp9, dsq, r, kmin, kmax, M,
  ...
  w = kmax[b]-kmin[b]+1;
  memcpy(g->bmr[b-a], s->bmmx + s->off[j], sizeof(int)*w);
  memcpy(g->bir[b-a], s->bimx + s->off[j], sizeof(int)*w);
  memcpy(g->bdr[b-a], s->bdmx + s->off[j], sizeof(int)*w);
  memcpy(g->ber[b-a], s->belmx+ s->off[j], sizeof(int)*w);
  for(r = b-1; r >= a; r--) {
    if(r == 0)
      cp9_chk_bwd_row0(cp9, dsq, kmin, kmax, M,
                       g->bmr[0], g->bir[0], g->bdr[0], g->ber[0],
                       g->bmr[1], g->bir[1], g->bdr[1], g->ber[1]);
    else if(r == L)
      cp9_chk_bwd_rowL(cp9, dsq, L, kmin, kmax, M,
                       g->bmr[r-a], g->bir[r-a], g->bdr[r-a], g->ber[r-a]);
    else
      cp9_chk_bwd_row(cp9, dsq, r, kmin, kmax, M,
                      g->bmr[r-a],   g->bir[r-a],   g->bdr[r-a],   g->ber[r-a],
                      g->bmr[r+1-a], g->bir[r+1-a], g->bdr[r+1-a], g->ber[r+1-a]);
  }
}

/* Compute posterior row i into g->pm/pi/pd (band-relative, k=kmin[i]..kmax[i]).
 * Verbatim from cp9_FB2HMMBandsP7B per-cell posterior. fmr/bmr are this row's
 * fwd/bck planes. */
static void
cp9seg_PostRow(cp9seg_t *g, CP9_t *hmm, ESL_DSQ *dsq, int i, int *kmin, int *kmax, int sc,
               int *fmr, int *fir, int *fdr, int *bmr, int *bir, int *bdr)
{
  int k, kp, kn, kx;
  int *pm=g->pm, *pi=g->pi, *pd=g->pd;

  if(i == 0) {
    /* row 0: pmx mmx[0][0], imx/dmx[0][0], then dmx[0][k] for k=1..kmax[0] */
    pm[0] = fmr[0] + bmr[0] - sc;
    pi[0] = -INFTY;
    pd[0] = -INFTY;
    kn = ESL_MAX(kmin[0], 1);
  ...
  if(INBAND(i,0)) {
    kp = 0;
    pm[kp] = ESL_MAX(fmr[kp] + bmr[kp] - sc, -INFTY);
    pi[kp] = ESL_MAX(fir[kp] + bir[kp] - hmm->isc[dsq[i]][0] - sc, -INFTY);
    pd[kp] = -INFTY;
  }
  kn = ESL_MAX(kmin[i], 1);
  kx = kmax[i];
  kp = kn - kmin[i];
  for(k = kn; k <= kx; k++, kp++) {
    pm[kp] = ESL_MAX(fmr[kp] + bmr[kp] - hmm->msc[dsq[i]][k] - sc, -INFTY);
    pi[kp] = ESL_MAX(fir[kp] + bir[kp] - hmm->isc[dsq[i]][k] - sc, -INFTY);
    pd[kp] = ESL_MAX(fdr[kp] + bdr[kp] - sc, -INFTY);
  }
}

/* The checkpointed band reduction: produces cp9b pn_min/pn_max bands exactly
 * as cp9_FB2HMMBandsP7B, but with O(sqrt(L)*avg_bw) memory.
 */
int
cp9_FB2HMMBandsP7B_chk(CP9_t *hmm, char *errbuf, ESL_DSQ *dsq, CP9Bands_t *cp9b,
                       int L, int M, double p_thresh, int *kmin, int *kmax,
                       int debug_level, int do_pnmono, int do_pnmono_print)
{
  int status;
  int thresh = Prob2Score(((1. - p_thresh)/2.), 1.);
  int *nset_m=NULL,*nset_i=NULL,*nset_d=NULL;
  int *xset_m=NULL,*xset_i=NULL,*xset_d=NULL;
  int *mass_m=NULL,*mass_i=NULL,*mass_d=NULL;
  int i, k, kp, kn, kx, sc, j;
  ...
  ESL_ALLOC(mass_m, sizeof(int)*(M+1)); ESL_ALLOC(mass_i, sizeof(int)*(M+1)); ESL_ALLOC(mass_d, sizeof(int)*(M+1));
  esl_vec_ISet(mass_m, M+1, -INFTY); esl_vec_ISet(mass_i, M+1, -INFTY); esl_vec_ISet(mass_d, M+1, -INFTY);
  ...
  if((s = cp9chk_Create(L, M, kmin, kmax, errbuf)) == NULL) { status = eslEMEM; goto ERROR; }

  /* DEBUG: compare my row kernels vs the original full F/B, cell-by-cell. */
  if(getenv("CP9_CKPT_FBCMP") != NULL) {
    CP9_MX *fr=CreateCP9Matrix(L,M), *br=CreateCP9Matrix(L,M);
    CP9_MX *fc=CreateCP9Matrix(L,M), *bc=CreateCP9Matrix(L,M);
    float rsc; int ii, kk, kp, w, fmis=0, bmis=0;
    cp9_ForwardP7B_OLD_WITH_EL(hmm, errbuf, fr, dsq, L, kmin, kmax, &rsc);
    cp9_BackwardP7B(hmm, errbuf, br, dsq, L, kmin, kmax, NULL);
    /* my forward into fc */
    cp9_chk_fwd_row0(hmm, kmin, kmax, M, fc->mmx[0], fc->imx[0], fc->dmx[0], fc->elmx[0], NULL);
    for(ii=1; ii<=L; ii++) cp9_chk_fwd_row(hmm, dsq, ii, kmin, kmax, M, fc->mmx[ii-1],fc->imx[ii-1],fc->dmx[ii-1],fc->elmx[ii-1], fc->mmx[ii],fc->imx[ii],fc->dmx[ii],fc->elmx[ii], NULL);
    /* my backward into bc */
    cp9_chk_bwd_rowL(hmm, dsq, L, kmin, kmax, M, bc->mmx[L],bc->imx[L],bc->dmx[L],bc->elmx[L]);
    for(ii=L-1; ii>=1; ii--) cp9_chk_bwd_row(hmm, dsq, ii, kmin, kmax, M, bc->mmx[ii],bc->imx[ii],bc->dmx[ii],bc->elmx[ii], bc->mmx[ii+1],bc->imx[ii+1],bc->dmx[ii+1],bc->elmx[ii+1]);
    cp9_chk_bwd_row0(hmm, dsq, kmin, kmax, M, bc->mmx[0],bc->imx[0],bc->dmx[0],bc->elmx[0], bc->mmx[1],bc->imx[1],bc->dmx[1],bc->elmx[1]);
    for(ii=0; ii<=L; ii++){ kp=0; for(kk=kmin[ii]; kk<=kmax[ii]; kk++,kp++){
      if(fr->mmx[ii][kp]!=fc->mmx[ii][kp]||fr->imx[ii][kp]!=fc->imx[ii][kp]||fr->dmx[ii][kp]!=fc->dmx[ii][kp]||fr->elmx[ii][kp]!=fc->elmx[ii][kp]){ if(fmis<12) fprintf(stderr,"#FBCMP F i=%d k=%d ref[M%d I%d D%d E%d] chk[M%d I%d D%d E%d]\n",ii,kk,fr->mmx[ii][kp],fr->imx[ii][kp],fr->dmx[ii][kp],fr->elmx[ii][kp],fc->mmx[ii][kp],fc->imx[ii][kp],fc->dmx[ii][kp],fc->elmx[ii][kp]); fmis++; }
      if(br->mmx[ii][kp]!=bc->mmx[ii][kp]||br->imx[ii][kp]!=bc->imx[ii][kp]||br->dmx[ii][kp]!=bc->dmx[ii][kp]||br->elmx[ii][kp]!=bc->elmx[ii][kp]){ if(bmis<12) fprintf(stderr,"#FBCMP B i=%d k=%d ref[M%d I%d D%d E%d] chk[M%d I%d D%d E%d]\n",ii,kk,br->mmx[ii][kp],br->imx[ii][kp],br->dmx[ii][kp],br->elmx[ii][kp],bc->mmx[ii][kp],bc->imx[ii][kp],bc->dmx[ii][kp],bc->elmx[ii][kp]); bmis++; }
    }}
    fprintf(stderr,"#FBCMP forward mismatches=%d backward mismatches=%d (L=%d M=%d)\n", fmis, bmis, L, M);
    /* posterior comparison */
    {
      CP9_MX *pr=CreateCP9Matrix(L,M); int scref=br->mmx[0][0]; int scchk=br->mmx[0][0]; int pmis=0;
      cp9_PosteriorP7B(dsq, errbuf, L, hmm, fr, br, pr, kmin, kmax);
      for(ii=0; ii<=L; ii++){ kp=0; for(kk=kmin[ii]; kk<=kmax[ii]; kk++,kp++){
        int pm,pi,pd;
        if(ii==0){ if(kk==0){pm=fr->mmx[0][0]+br->mmx[0][0]-scchk; pi=-INFTY; pd=-INFTY;} else {pm=-INFTY; pi=-INFTY; pd=fr->dmx[0][kp]+br->dmx[0][kp]-scchk;} }
        else if(kk==0){ pm=ESL_MAX(fr->mmx[ii][kp]+br->mmx[ii][kp]-scchk,-INFTY); pi=ESL_MAX(fr->imx[ii][kp]+br->imx[ii][kp]-hmm->isc[dsq[ii]][0]-scchk,-INFTY); pd=-INFTY; }
        else { pm=ESL_MAX(fr->mmx[ii][kp]+br->mmx[ii][kp]-hmm->msc[dsq[ii]][kk]-scchk,-INFTY); pi=ESL_MAX(fr->imx[ii][kp]+br->imx[ii][kp]-hmm->isc[dsq[ii]][kk]-scchk,-INFTY); pd=ESL_MAX(fr->dmx[ii][kp]+br->dmx[ii][kp]-scchk,-INFTY); }
        if(pm!=pr->mmx[ii][kp]||pi!=pr->imx[ii][kp]||pd!=pr->dmx[ii][kp]){ if(pmis<12) fprintf(stderr,"#PCMP i=%d k=%d ref[M%d I%d D%d] mine[M%d I%d D%d]\n",ii,kk,pr->mmx[ii][kp],pr->imx[ii][kp],pr->dmx[ii][kp],pm,pi,pd); pmis++; }
      }}
      fprintf(stderr,"#PCMP posterior mismatches=%d scref=%d\n", pmis, scref);
      /* run MY exact sweep over reference pmx pr; compare pn to reference */
      {
        int *Mn,*Mx,*In,*Ix,*Dn,*Dx,*ns_m,*ns_i,*ns_d,*xs_m,*xs_i,*xs_d,*ma_m,*ma_i,*ma_d;
        int *Rmn,*Rmx;
        int thr=Prob2Score(((1.-p_thresh)/2.),1.); int sc2=br->mmx[0][0]; int kkk,iii,smis=0;
        #define AL(p) ESL_ALLOC(p,sizeof(int)*(M+1))
        AL(Mn);AL(Mx);AL(In);AL(Ix);AL(Dn);AL(Dx);AL(ns_m);AL(ns_i);AL(ns_d);AL(xs_m);AL(xs_i);AL(xs_d);AL(ma_m);AL(ma_i);AL(ma_d);AL(Rmn);AL(Rmx);
        /* reference pn via cp9_FB2HMMBandsP7B (also refills pr with ref pmx) */
        cp9_FB2HMMBandsP7B(hmm, errbuf, dsq, fr, br, pr, cp9b, L, M, p_thresh, 0, kmin, kmax, 0, FALSE, FALSE);
        for(kkk=0;kkk<=M;kkk++){ Rmn[kkk]=cp9b->pn_min_m[kkk]; Rmx[kkk]=cp9b->pn_max_m[kkk]; }
        esl_vec_ISet(ma_m,M+1,-INFTY);esl_vec_ISet(ma_i,M+1,-INFTY);esl_vec_ISet(ma_d,M+1,-INFTY);
        esl_vec_ISet(ns_m,M+1,FALSE);esl_vec_ISet(ns_i,M+1,FALSE);esl_vec_ISet(ns_d,M+1,FALSE);
        esl_vec_ISet(xs_m,M+1,FALSE);esl_vec_ISet(xs_i,M+1,FALSE);esl_vec_ISet(xs_d,M+1,FALSE);
        /* MIN */
        for(iii=0;iii<=L;iii++){
          if(iii==0){ if((ma_m[0]=pr->mmx[0][0])>thr){Mn[0]=0;ns_m[0]=TRUE;} ma_i[0]=-INFTY; ma_d[0]=-INFTY;
            kn=ESL_MAX(kmin[0],1);kx=kmax[0];kp=kn-kmin[0]; for(kkk=kn;kkk<=kx;kkk++,kp++){ if((ma_d[kkk]=pr->dmx[0][kp])>thr){Dn[kkk]=0;ns_d[kkk]=TRUE;} } }
          else { kkk=0; if(INBAND(iii,0)){kp=0; if(!ns_m[0]){if((ma_m[0]=ILogsum(ma_m[0],pr->mmx[iii][kp]))>thr){Mn[0]=iii;ns_m[0]=TRUE;}} if(!ns_i[0]){if((ma_i[0]=ILogsum(ma_i[0],pr->imx[iii][kp]))>thr){In[0]=iii;ns_i[0]=TRUE;}} }
            kn=ESL_MAX(kmin[iii],1);kx=kmax[iii];kp=kn-kmin[iii]; for(kkk=kn;kkk<=kx;kkk++,kp++){
              if(!ns_m[kkk]){if((ma_m[kkk]=ILogsum(ma_m[kkk],pr->mmx[iii][kp]))>thr){Mn[kkk]=iii;ns_m[kkk]=TRUE;}}
              if(!ns_i[kkk]){if((ma_i[kkk]=ILogsum(ma_i[kkk],pr->imx[iii][kp]))>thr){In[kkk]=iii;ns_i[kkk]=TRUE;}}
              if(!ns_d[kkk]){if((ma_d[kkk]=ILogsum(ma_d[kkk],pr->dmx[iii][kp]))>thr){Dn[kkk]=iii;ns_d[kkk]=TRUE;}} } } }
        /* MAX */
        esl_vec_ISet(ma_m,M+1,-INFTY);esl_vec_ISet(ma_i,M+1,-INFTY);esl_vec_ISet(ma_d,M+1,-INFTY);
        for(iii=L;iii>=1;iii--){ kp=0; for(kkk=kmin[iii];kkk<=kmax[iii];kkk++,kp++){
          if(!xs_m[kkk]){if((ma_m[kkk]=ILogsum(ma_m[kkk],pr->mmx[iii][kp]))>thr){Mx[kkk]=iii;xs_m[kkk]=TRUE;}}
          if(!xs_i[kkk]){if((ma_i[kkk]=ILogsum(ma_i[kkk],pr->imx[iii][kp]))>thr){Ix[kkk]=iii;xs_i[kkk]=TRUE;}}
          if(!xs_d[kkk]){if((ma_d[kkk]=ILogsum(ma_d[kkk],pr->dmx[iii][kp]))>thr){Dx[kkk]=iii;xs_d[kkk]=TRUE;}} } }
        if(INBAND(0,0)){ if(!xs_m[0]){if((ma_m[0]=ILogsum(ma_m[0],pr->mmx[0][0]))>thr){Mx[0]=0;xs_m[0]=TRUE;}} }
        kn=ESL_MAX(kmin[0],1);kx=kmax[0];kp=kn-kmin[0]; for(kkk=kn;kkk<=kx;kkk++,kp++){ if(!xs_d[kkk]){if((ma_d[kkk]=ILogsum(ma_d[kkk],pr->dmx[0][kp]))>thr){Dx[kkk]=0;xs_d[kkk]=TRUE;}} }
        /* finalize like original */
        for(kkk=0;kkk<=M;kkk++){
          if((!ns_m[kkk])||(!xs_m[kkk])||(Mx[kkk]<Mn[kkk])){Mn[kkk]=Mx[kkk]=-1;}
          if((!ns_i[kkk])||(!xs_i[kkk])||(Ix[kkk]<In[kkk])){In[kkk]=Ix[kkk]=-1;}
          if((!ns_d[kkk])||(!xs_d[kkk])||(Dx[kkk]<Dn[kkk])){Dn[kkk]=Dx[kkk]=-1;}
        }
        Dn[0]=Dx[0]=-1;
        for(kkk=0;kkk<=M;kkk++){ if(Mn[kkk]!=Rmn[kkk]||Mx[kkk]!=Rmx[kkk]){ if(smis<12)fprintf(stderr,"#SWEEP k=%d M ref[%d,%d] mysweep[%d,%d]\n",kkk,Rmn[kkk],Rmx[kkk],Mn[kkk],Mx[kkk]); smis++; } }
        fprintf(stderr,"#SWEEP my-sweep-over-refpmx vs ref pn: M-mismatches(partial)=%d (sc2=%d thr=%d)\n", smis, sc2, thr);
        free(Mn);free(Mx);free(In);free(Ix);free(Dn);free(Dx);free(ns_m);free(ns_i);free(ns_d);free(xs_m);free(xs_i);free(xs_d);free(ma_m);free(ma_i);free(ma_d);free(Rmn);free(Rmx);
        #undef AL
      }
      FreeCP9Matrix(pr);
    }
    /* verify rolling fills + checkpoint store + segment recompute vs fc/bc */
    {
      int scx; int jj, rr, kk2, kp2, fcmis=0, bcmis=0;
      cp9seg_t *gg;
      cp9chk_FwdFill(s, hmm, dsq, kmin, kmax, errbuf);
      cp9chk_BwdFill(s, hmm, dsq, kmin, kmax, &scx, errbuf);
      /* checkpoint store rows vs fc/bc */
      for(jj=0; jj<s->nbnd; jj++){ rr=s->bnd[jj]; kp2=0; for(kk2=kmin[rr];kk2<=kmax[rr];kk2++,kp2++){
        if(s->fmmx[s->off[jj]+kp2]!=fc->mmx[rr][kp2]||s->fdmx[s->off[jj]+kp2]!=fc->dmx[rr][kp2]){ if(fcmis<8) fprintf(stderr,"#STORE Fckpt row=%d k=%d store[M%d D%d] fc[M%d D%d]\n",rr,kk2,s->fmmx[s->off[jj]+kp2],s->fdmx[s->off[jj]+kp2],fc->mmx[rr][kp2],fc->dmx[rr][kp2]); fcmis++; }
        if(s->bmmx[s->off[jj]+kp2]!=bc->mmx[rr][kp2]||s->bdmx[s->off[jj]+kp2]!=bc->dmx[rr][kp2]){ if(bcmis<8) fprintf(stderr,"#STORE Bckpt row=%d k=%d store[M%d D%d] bc[M%d D%d]\n",rr,kk2,s->bmmx[s->off[jj]+kp2],s->bdmx[s->off[jj]+kp2],bc->mmx[rr][kp2],bc->dmx[rr][kp2]); bcmis++; }
      }}
      fprintf(stderr,"#STORE Fckpt-mis=%d Bckpt-mis=%d scx=%d (vs scref above)\n", fcmis, bcmis, scx);
      /* segment materialize vs fc/bc */
      gg = cp9seg_Create(s, kmin, kmax, errbuf);
      { int segfmis=0, segbmis=0;
        for(jj=1; jj<s->nbnd; jj++){ int aa=s->bnd[jj-1], bb=s->bnd[jj];
          cp9seg_Fill(gg, s, jj, hmm, dsq, kmin, kmax);
          for(rr=aa; rr<=bb; rr++){ kp2=0; for(kk2=kmin[rr];kk2<=kmax[rr];kk2++,kp2++){
            if(gg->fmr[rr-aa][kp2]!=fc->mmx[rr][kp2]){ if(segfmis<8) fprintf(stderr,"#SEG F seg%d row=%d k=%d seg=%d fc=%d\n",jj,rr,kk2,gg->fmr[rr-aa][kp2],fc->mmx[rr][kp2]); segfmis++; }
            if(gg->bmr[rr-aa][kp2]!=bc->mmx[rr][kp2]){ if(segbmis<8) fprintf(stderr,"#SEG B seg%d row=%d k=%d seg=%d bc=%d\n",jj,rr,kk2,gg->bmr[rr-aa][kp2],bc->mmx[rr][kp2]); segbmis++; }
          }}
        }
        fprintf(stderr,"#SEG seg-F-mis=%d seg-B-mis=%d\n", segfmis, segbmis);
      }
      cp9seg_Destroy(gg);
    }
    FreeCP9Matrix(fr);FreeCP9Matrix(br);FreeCP9Matrix(fc);FreeCP9Matrix(bc);
  }

  if(getenv("CP9_CKPT_VERBOSE") != NULL) {
    int64_t full_ncells = 0; int r;
    for(r = 0; r <= L; r++) full_ncells += (kmax[r]-kmin[r]+1);
    /* non-ckpt holds 3 banded CP9_MX of 16*full_ncells B each; ckpt holds 2
  ...
  if((status = cp9chk_BwdFill(s, hmm, dsq, kmin, kmax, &sc, errbuf)) != eslOK) goto ERROR;
  if((g = cp9seg_Create(s, kmin, kmax, errbuf)) == NULL) { status = eslEMEM; goto ERROR; }

  /* === MIN sweep: ascending i (row 0 first, then 1..L) === */
  /* row 0 boundary contributions (handled inside segment 1) */
  for(j = 1; j < s->nbnd; j++) {
    int a = s->bnd[j-1], b = s->bnd[j];
    int istart = (j == 1) ? 0 : a+1;
    cp9seg_Fill(g, s, j, hmm, dsq, kmin, kmax);
    for(i = istart; i <= b; i++) {
      int ri = i - a;
      cp9seg_PostRow(g, hmm, dsq, i, kmin, kmax, sc,
      ...
      if(i == 0) {
        ...
        if(INBAND(i,0)) {
          kp = 0;
          if(! nset_m[0]) { if((mass_m[0] = ILogsum(mass_m[0], g->pm[kp])) > thresh) { cp9b->pn_min_m[0] = i; nset_m[0] = TRUE; } }
          if(! nset_i[0]) { if((mass_i[0] = ILogsum(mass_i[0], g->pi[kp])) > thresh) { cp9b->pn_min_i[0] = i; nset_i[0] = TRUE; } }
        }
        kn = ESL_MAX(kmin[i], 1); kx = kmax[i]; kp = kn - kmin[i];
        for(k = kn; k <= kx; k++, kp++) {
          if(! nset_m[k]) { if((mass_m[k] = ILogsum(mass_m[k], g->pm[kp])) > thresh) { cp9b->pn_min_m[k] = i; nset_m[k] = TRUE; } }
          if(! nset_i[k]) { if((mass_i[k] = ILogsum(mass_i[k], g->pi[kp])) > thresh) { cp9b->pn_min_i[k] = i; nset_i[k] = TRUE; } }
          if(! nset_d[k]) { if((mass_d[k] = ILogsum(mass_d[k], g->pd[kp])) > thresh) { cp9b->pn_min_d[k] = i; nset_d[k] = TRUE; } }
        }
      }
    }
  }
  ...
  for(j = s->nbnd - 1; j >= 1; j--) {
    int a = s->bnd[j-1], b = s->bnd[j];
    int istart = b;
    int iend   = (j == 1) ? 1 : a+1;  /* row 0 handled separately after */
    cp9seg_Fill(g, s, j, hmm, dsq, kmin, kmax);
    for(i = istart; i >= iend; i--) {
      ...
      for(k = kmin[i]; k <= kmax[i]; k++, kp++) {
        if(! xset_m[k]) { if((mass_m[k] = ILogsum(mass_m[k], g->pm[kp])) > thresh) { cp9b->pn_max_m[k] = i; xset_m[k] = TRUE; } }
        if(! xset_i[k]) { if((mass_i[k] = ILogsum(mass_i[k], g->pi[kp])) > thresh) { cp9b->pn_max_i[k] = i; xset_i[k] = TRUE; } }
        if(! xset_d[k]) { if((mass_d[k] = ILogsum(mass_d[k], g->pd[kp])) > thresh) { cp9b->pn_max_d[k] = i; xset_d[k] = TRUE; } }
      }
      if(j == 1 && i == 1) {
        /* row 0 boundary (descending order: after row 1) */
        cp9seg_PostRow(g, hmm, dsq, 0, kmin, kmax, sc,
                       g->fmr[0], g->fir[0], g->fdr[0], g->bmr[0], g->bir[0], g->bdr[0]);
        if(INBAND(0,0)) {
          if(! xset_m[0]) { if((mass_m[0] = ILogsum(mass_m[0], g->pm[0])) > thresh) { cp9b->pn_max_m[0] = 0; xset_m[0] = TRUE; } }
        }
        kn = ESL_MAX(kmin[0], 1); kx = kmax[0]; kp = kn - kmin[0];
        for(k = kn; k <= kx; k++, kp++) {
          if(!xset_d[k]) { if((mass_d[k] = ILogsum(mass_d[k], g->pd[kp])) > thresh) { cp9b->pn_max_d[k] = 0; xset_d[k] = TRUE; } }
        }
      }
    }
  }

  /* finalize (verbatim from cp9_FB2HMMBandsP7B) */
  {
    int mset, dset;
    for(k = 0; k <= M; k++) {
      mset = dset = TRUE;
  ...
  if(do_pnmono) pn_match_bands_enforce_monotone(cp9b->pn_min_m, cp9b->pn_max_m, M, L, do_pnmono_print, "fb2hmm_p7b_chk");

  /* DEBUG: compare chk pn arrays vs the original full-matrix reduction. */
  if(getenv("CP9_CKPT_DEBUG") != NULL) {
    int *tmn,*tmx_m,*tin,*tix,*tdn,*tdx;
    CP9_MX *fmx=NULL,*bmx=NULL,*pmx=NULL; float rsc; int kk, nmis=0;
    ...
    fmx=CreateCP9Matrix(1,M); bmx=CreateCP9Matrix(1,M); pmx=CreateCP9Matrix(1,M);
    cp9_ForwardP7B_OLD_WITH_EL(hmm, errbuf, fmx, dsq, L, kmin, kmax, &rsc);
    cp9_BackwardP7B(hmm, errbuf, bmx, dsq, L, kmin, kmax, NULL);
    cp9_FB2HMMBandsP7B(hmm, errbuf, dsq, fmx, bmx, pmx, cp9b, L, M, p_thresh, 0, kmin, kmax, 0, do_pnmono, do_pnmono_print);
    for(kk=0;kk<=M;kk++){
      if(tmn[kk]!=cp9b->pn_min_m[kk]||tmx_m[kk]!=cp9b->pn_max_m[kk]) { if(nmis<20) fprintf(stderr,"#CKPTDBG k=%d M ref[%d,%d] chk[%d,%d]\n",kk,cp9b->pn_min_m[kk],cp9b->pn_max_m[kk],tmn[kk],tmx_m[kk]); nmis++; }
      if(tin[kk]!=cp9b->pn_min_i[kk]||tix[kk]!=cp9b->pn_max_i[kk]) { if(nmis<20) fprintf(stderr,"#CKPTDBG k=%d I ref[%d,%d] chk[%d,%d]\n",kk,cp9b->pn_min_i[kk],cp9b->pn_max_i[kk],tin[kk],tix[kk]); nmis++; }
      if(tdn[kk]!=cp9b->pn_min_d[kk]||tdx[kk]!=cp9b->pn_max_d[kk]) { if(nmis<20) fprintf(stderr,"#CKPTDBG k=%d D ref[%d,%d] chk[%d,%d]\n",kk,cp9b->pn_min_d[kk],cp9b->pn_max_d[kk],tdn[kk],tdx[kk]); nmis++; }
    }
    fprintf(stderr,"#CKPTDBG total pn mismatches: %d (M=%d L=%d)\n", nmis, M, L);
    ...
    FreeCP9Matrix(fmx); FreeCP9Matrix(bmx); FreeCP9Matrix(pmx);
 ...
 ERROR:
  if(g) cp9seg_Destroy(g);
  if(s) cp9chk_Destroy(s);
...
/* Function: cp9_FBMatrices2BandsP7B_chk()
 *
 * Checkpointed drop-in for cp9_FBMatrices2BandsP7B (non-truncated path only).
 * Produces the same cp9b bands via cp9_FB2HMMBandsP7B_chk + the identical
 * downstream HMM2ij/GrowHD/ij2d tail. No CP9 matrices required.
 */
int
cp9_FBMatrices2BandsP7B_chk(CM_t *cm, char *errbuf, CP9_t *cp9, ESL_DSQ *dsq, CP9Bands_t *cp9b,
                            ...
                            int debug_level, int do_pnmono, int do_pnmono_print)
{
  ...
  if(do_trunc) ESL_FAIL(eslEINCOMPAT, errbuf, "cp9_FBMatrices2BandsP7B_chk: truncated path not implemented (Phase 2).");
  if(use_sums) ESL_FAIL(eslEINCOMPAT, errbuf, "cp9_FBMatrices2BandsP7B_chk: use_sums not supported.");

  /* Step 1+2: checkpointed F/B -> HMM bands. */
  if((status = cp9_FB2HMMBandsP7B_chk(cp9, errbuf, dsq, cp9b, L, cp9b->hmm_M,
  ...
  /* Step 2c: non-trunc valid arrays. */
...
/*****************************************************************
 * FLOAT: Checkpoint store + fills (mirror of cp9chk_t).
 *****************************************************************/
