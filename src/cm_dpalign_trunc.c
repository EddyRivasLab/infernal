...
static int fast_tr_cyk_align_hb (CM_t *cm, char *errbuf, ESL_DSQ *dsq, int i0, int j0, float size_limit, CM_TR_HB_MX *mx,
				 CM_TR_HB_SHADOW_MX *shmx, int *ret_b, float *ret_bsc, char *ret_bmode, float *ret_sc);
static int fast_tr_alignT_hb(CM_t *cm, char *errbuf, ESL_DSQ *dsq, Parsetree_t *tr, 
			     int i0, int j0, CM_TR_HB_MX *mx, CM_TR_HB_SHADOW_MX *shmx, 
			     int do_optacc, CM_TR_HB_MX *post_mx, float size_limit, float *ret_sc);


/* Function: fast_tr_cyk_align_hb()
 ...
 *           in bmode, telling fast_tr_hb_alignT() to check state
 ...
 */
int
fast_tr_cyk_align_hb(CM_t *cm, char *errbuf,  ESL_DSQ *dsq, int i0, int j0, float size_limit, CM_TR_HB_MX *mx, 
		     CM_TR_HB_SHADOW_MX *shmx, int *ret_b, float *ret_bsc, char *ret_bmode, float *ret_sc)
{
  int      status;
  ...
  for (v = cm->M-1; v >= 0; v--) {
	...
	if(do_T_v) { 
       ...
       * the shadow matrix telling fast_alignT() to use the b we
       * return.
       */
      if (v == 0) { 
	assert(do_J_v);
  ...
  /*FILE *fp1; fp1 = fopen("tmp.amx", "w");   cm_tr_hb_mx_Dump(fp1, mx); fclose(fp1);*/
  /*FILE *fp2; fp2 = fopen("tmp.ashmx", "w"); cm_tr_hb_shadow_mx_Dump(fp2, cm, shmx); fclose(fp2);*/
  ...
  *ret_sc    = sc;
  ...
  ESL_DPRINTF1(("fast_tr_cyk_align_hb return sc: %f\n", sc));
  printf("fast_tr_cyk_align_hb return sc: %.4f (bsc: %.4f)\n", sc, bsc);
  return eslOK;
 ...
 */
int
fast_tr_alignT_hb(CM_t *cm, char *errbuf, ESL_DSQ *dsq, Parsetree_t *tr, 
		  int i0, int j0, CM_TR_HB_MX *mx, CM_TR_HB_SHADOW_MX *shmx, 
		  int do_optacc, CM_TR_HB_MX *post_mx, float size_limit, float *ret_sc)
{
  int status;
  ...
  int       v,j,d,i,z;		/* indices for state, j, subseq len */
  ...
  /* contract check */
  if(dsq == NULL)                  ESL_FAIL(eslEINCOMPAT, errbuf, "fast_tr_alignT_hb(), dsq == NULL.\n");
  if(cm->cp9b == NULL)             ESL_FAIL(eslEINCOMPAT, errbuf, "fast_tr_alignT_hb(), cm->cp9b == NULL.\n");
  if(do_optacc && post_mx == NULL) ESL_FAIL(eslEINCOMPAT, errbuf, "fast_tr_alignT_hb(), do_optacc == TRUE but post_mx == NULL.\n");
  ...
  if(do_optacc) {
    ...
    status = fast_tr_cyk_align_hb(cm, errbuf, dsq, i0, j0, 
 ...
 *
 * Purpose: Wrapper for the fast_tr_alignT_hb() routine - solve a full
 ...
 *           code string.
 *           
 *           Identical to FastTrAlign() but HMM bands are used here.
 * 
...
int
FastTrAlignHB(CM_t *cm, char *errbuf, ESL_RANDOMNESS *r, ESL_DSQ *dsq, int i0, int j0, float size_limit, CM_TR_HB_MX *mx, CM_TR_HB_SHADOW_MX *shmx, 
	      int do_optacc, int do_sample, CM_TR_HB_MX *post_mx, Parsetree_t **ret_tr, char **ret_pcode, float *ret_sc, float *ret_ins_sc)
{
  ...
  if(!do_sample) { /* if do_sample, we already have a parsetree */
    ...
    if((status = fast_tr_alignT_hb(cm, errbuf, dsq, tr, i0, j0, mx, shmx, do_optacc, post_mx, size_limit, &sc)) != eslOK) return status;
  }
...
main(int argc, char **argv)
{
  ...
  for (i = 0; i < N; i++)
	...
	cm->cp9b->thresh2 = ESL_MAX(0.25, cm->cp9b->thresh2);
      ...
      trhbshmx = cm_tr_hb_shadow_mx_Create(cm);
      esl_stopwatch_Start(w);
      if((status = FastTrAlignHB(cm, errbuf, NULL, dsq, 1, L, size_limit, trhbmx, trhbshmx, FALSE, FALSE, NULL, &tr, NULL, &sc, NULL)) != eslOK) cm_Fail(errbuf);
      printf("%4d %-30s %10.4f bits ", (i+1), "FastTrAlignHB(): ", sc);
      ParsetreeDump(stdout, tr, cm, dsq, NULL, NULL);
      ParsetreeScore(cm, NULL, NULL, tr, dsq, FALSE, &parsetree_sc, &parsetree_struct_sc, NULL, NULL, NULL);
      printf("Parsetree score      : %.4f           (FULL LENGTH)\n", parsetree_sc);
      esl_stopwatch_Stop(w);
      esl_stopwatch_Display(stdout, w, " CPU time: ");
      /*********************End FastTrAlignHB*****************************/
