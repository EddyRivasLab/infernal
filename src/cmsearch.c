...
static ESL_OPTIONS options[] = {
  ...
  { "--novitband", eslARG_NONE,   FALSE, NULL, NULL,    NULL,  NULL, NULL, "disable default vitband (F4/F5 unbanded)",                                   6 },
...
process_commandline(int argc, char **argv, ESL_GETOPTS **ret_go, char **ret_cmfile, char **ret_seqfile)
{
  ...
  if (do_dev || esl_opt_GetBoolean(go, "-h"))
    {
      ...
      cm_banner(stdout, "cmsearch", banner);  // use progname not argv[0]: versioning, not invocation
  ...
  if (esl_opt_GetBoolean(go, "--version"))
    {
      ...
      esl_printf("%s %s\n", "cmsearch", INFERNAL_VERSION);  // use progname here: versioning, not invocation
      exit(0);
    }
  ...
  if ((*ret_seqfile = esl_opt_GetArg(go, 2)) == NULL)  { esl_fprintf(stderr, "Failed to get <seqdb> argument on command line.\n");  goto ERROR; }
  
  if (strcmp(*ret_seqfile, "-") == 0) { esl_fprintf(stderr, "Can't read <seqdb> from stdout: don't use '-'\n"); goto ERROR; }
  ...
  if (esl_opt_IsOn(go, "--glist")) { 
    esl_fprintf(stderr, "Failed to parse command line: --glist is an invalid cmsearch option (it only works with cmscan)\n");
    goto ERROR;
  }    
  ...
  if((esl_opt_IsUsed(go, "--fmt")) && (esl_opt_GetInteger(go, "--fmt") == 2)) { 
    esl_fprintf(stderr, "--fmt 2 only makes sense with cmscan, because cmsearch can't determine overlaps\n");
    goto ERROR;
  }
