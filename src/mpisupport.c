...
expinfo_MPIPackSize(ExpInfo_t *exp, MPI_Comm comm, int *ret_n)
{
  ...
  /* nrandhits, is_valid */
  status = MPI_Pack_size(1, MPI_LONG, comm, &sz);   n += 1*sz;  if (status != 0) ESL_XEXCEPTION(eslESYS, "pack size failed");
  /* dbsize */
  status = MPI_Pack_size(1, MPI_DOUBLE, comm, &sz); n += 5*sz; if (status != 0) ESL_XEXCEPTION(eslESYS, "pack size failed");
  /* mu_orig, mu_extrap, lambda, tailp, cur_eff_dbsize */
...
cm_hit_MPIPackSize(CM_HIT *hit, MPI_Comm comm, int *ret_n)
{
  ...
  if (MPI_Pack_size(5,            MPI_INT,    comm, &sz) != 0) ESL_XEXCEPTION(eslESYS, "pack size failed"); n += sz;  /* in_rc, root, mode, pass_idx, hmmonly */
