...
static ESL_OPTIONS options[] = {
  ...
  { "--Sfinal", eslARG_INT,   "3","0<n<4",NULL,  NULL,  NULL, NULL, "set final stage as stage <n>", 1 },
...
main(int argc, char **argv)
{
  ...
  int              Sfinal   = esl_opt_GetInteger(go, "--Sfinal");
...
PIPELINE:
  ...
  if (o_glbf_all) {
    fclose(S0_OFILE);
    fclose(S1_OFILE);
    if(Sfinal >=2) fclose(S2_OFILE);
    if(Sfinal >=3) fclose(S3_OFILE);
  }
