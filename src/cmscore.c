...
static int summarize_search_options(const struct cfg_s *cfg, CM_t *cm);
...
initialize_cm_for_search(const ESL_GETOPTS *go, const struct cfg_s *cfg, char *errbuf, CM_t *cm)
{
  ...
  if(cfg->my_rank == 0) 
	...
	printf("=================================\n");
      ...
      printf("Stage %2d search:\n", (cfg->s+1));
      summarize_search_options(cfg, cm);
    }
  return eslOK;
}
...
/* Function: summarize_search_options
 * Date:     EPN, Fri Jan 25 09:26:32 2008
 * Purpose:  Print out search options in pretty format. 
 */
int summarize_search_options(const struct cfg_s *cfg, CM_t *cm)
{
  printf("---------------------------------\n");
  /* Algorithm */
  if(cm->search_opts & CM_SEARCH_INSIDE)
    printf("Algorithm:               Inside scanning\n");
  else if(cm->search_opts & CM_SEARCH_HMMVITERBI) 
    printf("Algorithm:               CP9 HMM Viterbi scanning\n");
  else 
    printf("Algorithm:               CYK Standard scanning\n");

  /* Bands */
  if(cm->search_opts & CM_SEARCH_HBANDED)
    {
      if(cm->search_opts & CM_SEARCH_SUMS)
	printf("Bands:                   CP9 HMM (sums)\n");
      else
	printf("Bands:                   CP9 HMM\n"); 
      printf("Tail loss:               %g\n", cm->tau);
