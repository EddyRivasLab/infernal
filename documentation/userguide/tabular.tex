...
\prog{cmscan}, the \ccode{--fmt 2} can additionally be used with
\ccode{--tblout} to produce the alternative \emph{format 2} target hits
...
hit list). If the \ccode{--oclan} option was enabled, the definition
...
\item[\emprog{anyidx:}]
For hits that have \ccode{=} in the \ccode{olp} field, this is the
index of the best scoring hit that overlaps with this hit.
For hits with either \ccode{*} or \ccode{^} in the \ccode{olp} field,
...
overlapping hit (the hit index given in the \ccode{anyidx} field), to
...
index given in the \ccode{anyidx} field) that overlaps with this hit,
to 4 significant digits. 
For hits with either \ccode{*} or \ccode{^} in the \ccode{olp} field,
this field will always be \ccode{-}.

\item[\emprog{winidx:}] 
For hits that have \ccode{=} in the \ccode{olp} field, this is either
\ccode{``} or the index of the best scoring hit that overlaps with this
hit that is marked as \ccode{^} in the \ccode{olp} field. If the value
is \ccode{``} it means that the best scoring hit that overlaps with
this hit that is marked as \ccode{^} in the \ccode{olp} field is
already listed in the \ccode{anyidx} field, which is usually the case.
For hits with either \ccode{*} or \ccode{^} in the \ccode{olp} field,
this field will always be \ccode{-}.

\item[\emprog{winfrct1:}]
For hits that have neither \ccode{-} nor \ccode{``} in the
\ccode{winidx} field, this is the fraction of the length of this hit
that overlaps with the best scoring overlapping hit marked with
\ccode{^} in the \ccode{olp} field (the hit index given in the
\ccode{winidx} field), to 4 significant digits.  For hits with either
\ccode{*} or \ccode{^} in the \ccode{olp} field, this field will
always be \ccode{-}.  For hits with \ccode{-} in the \ccode{winidx}
field, this field will always be \ccode{-}.  For hits with \ccode{``}
in the \ccode{winidx} field, this field will always be \ccode{``}.

\item[\emprog{winfrct2:}]
For hits that have neither \ccode{-} nor \ccode{``} in the
\ccode{winidx} field, this is the
fraction of the length of the best scoring overlapping hit marked with
\ccode{^} in the \ccode{olp} field (the hit
index given in the \ccode{anyidx} field) that overlaps with this hit,
to 4 significant digits. 
