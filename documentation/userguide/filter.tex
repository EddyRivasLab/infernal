...
\prog{cmsearch} can be run with non-calibrated models but the the
...
When \prog{cmsearch} sets it's thresholds for each round of the
...
As described above, \prog{cmcalibrate} automatically determines the
...
\prog{--fil-Smax-hmm <x>}. With this option for values of $<x> << 1$
