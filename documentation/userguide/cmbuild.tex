...
estimates using mixture Dirichlet priors\citep{Durbin98,NawrockiEddy07}. 
...
\otext{<x>} (from 0 to 1.0) by the \prog{--gapthresh <x>} option.  The
...
\otext{\#=GC RF} line, in Stockholm format) and using the \prog{--rf}
...
and insert columns with \otext{.}'s. Remember that spaces aren't
...
\citep{Durbin98, NawrockiEddy07}. You can provide your own prior as a
file, using the \prog{--prior <f>} option.
...
section~\ref{section:format} of this guide.
