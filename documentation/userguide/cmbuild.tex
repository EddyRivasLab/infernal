...
posteriori parameters, using Dirichlet priors. A current limitation of
\prog{cmbuild} is that it uses simple, uninformative plus-one
(Laplace) Dirichlet priors, rather than priors trained on real data.
This means that \software{Infernal} currently only performs acceptably
well for alignments of many sequences (100 or more), where the counts
observed in the data are sufficient to determine all the parameters
well, without much effect of the prior.
...
\begin{srefaq}{Why does cmbuild have a \prog{--informat} option, if it only
...
By default, the sequences are weighted, instead of counting as 1
observation each, such that similar sequences are downweighted. This
partially compensates for phylogenetic and sampling bias in the data,
where some sequences are oversampled relative to the rest.  Weights
are calculated by the Gerstein/Sonnhammer/Chothia algorithm
\cite{Gerstein94}. (The \prog{--wgsc} option forces G/S/C weights, but
is redundant since that's the default.)

\begin{srefaq}{Why is cmbuild taking so much time?} The weighting algorithm
...
To turn weighting off (e.g. set all weights to 1.0), use the
\prog{--wnone} option.

Some alignment file formats allow sequence weights to be given in the
file. This includes Stockholm format, which has \verb+#=GS WT+ weight
annotations. Normally \prog{cmbuild} ignores any such input weights.
The \prog{--wgiven} option tells \prog{cmbuild} to use them.  This
lets you set the weights with any external procedure you like; for
example, the \prog{weight} utility program in \software{squid}
implements some common weighting algorithms, including the fast $O(N)$
Henikoff position-based weights \cite{Henikoff94b}. 

If for some reason you put more than one weighting option on the
command line, the last one you give is used.
...
the consensus. At \prog{--gapthresh 0.0}, none of them are (probably
not a good idea). 
...
estimated probabilities $p$ using a simple Dirichlet prior:
 
\[
\hat{p}_x = \frac{c_x + \alpha_x}{\sum_y c_y + \alpha_y}
\]

where the Dirichlet parameters $\alpha$ are all set to 1.0 (e.g. an
uninformative plus-one, or Laplace, prior).

There are currently no available options at this stage, but this will
change in the future when better priors are made.
...
You can override this and provide your own name with the \prog{-n <s>}
...
to have the same name! Therefore when running \prog{cmbuild} on a
...
point with an error. Attempting to set the \prog{--n} option for an
