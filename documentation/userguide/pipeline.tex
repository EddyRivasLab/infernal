...
programs by using the \prog{--nonull3} option. E-value parameters are
sensitive to whether or not the null3 correction was applied during
calibration, so a CM must carry a null3-off calibration of its own in
order for \prog{--nonull3} E-values to be meaningful; as of the
\otext{INFERNAL1/d} format, a single CM stores \emph{both} sets (see
section~\ref{section:calibration-limitations} below), so this happens
automatically and \prog{--nonull3} searches do not require any extra
calibration step.

\subsubsection{Automatic E-value calibration, store-both
  \prog{--nonull3} parameters, and known limitations.}
\label{section:calibration-limitations}

As of Infernal 1.2, \prog{cmbuild} calibrates E-value statistical
parameters automatically, using an analytical predictor
(\prog{cm\_FastCalibrate}) in place of \prog{cmcalibrate}'s older
Monte Carlo simulation. Every default-construction CM that comes out
of \prog{cmbuild} is therefore immediately ready for
\prog{cmsearch}/\prog{cmscan}; see
section~\ref{section:cmbuild} for details, including the small set of
non-default \prog{cmbuild} construction options under which automatic
calibration is not possible and \prog{cmcalibrate} is still required.

The predictor computes two E-value parameter sets and stores them both
in the CM file: the default set, for searches with the null3 biased
composition correction on, and a null3-off set, used automatically
when a search is run with \prog{--nonull3}. If a CM lacks the
null3-off set (for example, an older CM file, or one produced by an
external tool), a \prog{--nonull3} search falls back to the null3-on
parameters and prints a one-time warning; this fallback is
approximate, since null3-on and null3-off score distributions are not
the same.

The default (null3-on), local-mode E-values produced by the predictor
were validated to be effectively identical to the values previously
obtained from \prog{cmcalibrate}, and were confirmed against the
\prog{rmark4h} benchmark. The following limitations apply to the
remaining, off-default combinations:

\begin{itemize}

\item \textbf{\prog{--nonull3} accuracy for glocal-mode search and
  larger models.} The predicted null3-off E-value parameters are
  accurate for local-mode search of most models, but become
  unreliable for glocal-mode search and for larger models (consensus
  length greater than a few hundred). In these cases the predicted
  \prog{--nonull3} E-values can be over-confident (too low) --
  by up to several orders of magnitude for large, low-complexity
  (AT-rich) models such as the large ribosomal RNAs. For accurate
  \prog{--nonull3} E-values on such models, run
  \prog{cmcalibrate --nonull3}, which computes and stores true
  null3-off parameters in the CM; the predicted values are then used
  only as a fallback, no longer as the parameters actually searched
  with. Default (null3-on) E-values are not affected by this
  limitation.

\item \textbf{Glocal-mode E-value accuracy for large and huge
  models.} More generally, glocal-mode E-values for large and huge
  models (consensus length greater than $\sim$1500) are less reliable
  than local-mode E-values, because the glocal score distributions are
  sparsely sampled and the exponential-tail fits are correspondingly
  ill-determined at that scale -- this is true of both the analytical
  predictor and \prog{cmcalibrate}. On the default (\prog{--eent})
  construction path, glocal E-value parameters can occasionally be
  wrong by 10 or more orders of magnitude \emph{relative to
  \prog{cmcalibrate}'s own tail fit}, but checked against real random-hit
  data (brief 26\_0422-105) the practical impact is much smaller than
  that figure suggests: across 6 representative huge families, the
  default-path predictor is within $\sim$300 bits of the empirical
  E=0.01 threshold in 10 of 12 (family, mode) cells and is permissive
  (false-positive-prone) in only 2 of 12 -- the 10-plus-orders-of-magnitude
  figure reflects comparisons against \prog{cmcalibrate}'s own reference
  fit, which for several of the largest models is itself unreliable
  (bulk-fit, not tail-fit; see below). Users who rely on glocal-mode
  E-values (\prog{cmsearch}/\prog{cmscan} \prog{-g}) for large or huge
  models should still treat them with real caution, since this remains
  checked only for huge-clen models and not for the tiny/small sizes
  where most real usage lives. Validating with \prog{cmcalibrate} is a
  reasonable mitigation up to consensus length $\sim$1900; beyond that,
  \prog{cmcalibrate}'s own default recipe cannot reliably calibrate the
  glocal tail either (random glocal hits are scarce enough that its
  default tail-fraction falls back onto the bulk of the score
  distribution, not a true tail), so it is not a dependable check for
  the largest models. For \prog{cmbuild --ere}/\prog{--enone} builds
  specifically, the huge (consensus length $\geq$ 1500) glocal STR
  predictor cells (ECMGC/ECMGI) use a two-tier design (brief
  26\_0422-101/103/107): the plain (non-\prog{ere}) ridge's own
  predicted $\lambda$ is used directly whenever it is physically
  plausible ($\lambda \le 0.1$, comparable to a normal non-degenerate
  huge-glocal fit); a separately refit cell (brief 26\_0422-101/103) is
  used only as a fallback, for the specific case where that $\lambda$
  is non-physical ($\lambda \le 0$ or $\lambda > 0.1$ -- impossible
  positive glocal $\mu$ and glocal $\lambda$ exceeding 1 were both
  observed in the training population). Checked against real random-hit
  data (brief 26\_0422-105/106), the plain ridge is \emph{more} accurate
  than the refit fallback cell in the large majority of these 12 cells,
  so restricting the fallback to only the physically-implausible cases
  is both more accurate and less permissive than always using it (an
  earlier version of this fix, brief 26\_0422-103, used the refit cell
  unconditionally and was found, empirically, to be worse than the
  plain ridge in most cells and more permissive than it in all of
  them -- corrected by brief 26\_0422-107). Where the fallback cell is
  used, its underlying training labels still carry real,
  convention-quality uncertainty (roughly 20--100\%, median $\sim$48\%,
  relative cross-seed spread) rather than being a precise measurement;
  this is comparable in kind to (if a bit above) what non-degenerate
  huge glocal fits show on their own (roughly 26--42\%, median
  $\sim$33\%).

\item \textbf{Local-Inside (default) E-values for AT-rich models.} For
  strongly AT-rich or low-complexity models, the default local-Inside
  E-values carry a small residual over-confidence (roughly two- to
  three-fold near the significance threshold) that the predictor
  cannot fully remove. This residual is within the intrinsic
  seed-to-seed variation of \prog{cmcalibrate} itself and does not
  materially affect the ranking of hits.
...
nucleotide.\footnote{As of version 1.1.5, truncated hits that end at
  ...
  \ccode{--anytrunc} from v1.1.4, use the \ccode{--inttrunc} option
  instead.}
