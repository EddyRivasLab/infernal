...
scales pretty well up to hundreds of processors. \prog{cmsearch}
scales all right. \prog{cmscan} scales poorly poorly, and probably
shouldn't be used on more than tens of processors at most. Improving
MPI scaling is one of our goals.

\begin{comment}
% Currently this won't work in Infernal, first error complains it 
% doesn't know where to find hmmer.h. I would try and fix it based
% on how HMMER does it, but currently it won't work in HMMER either...
% So I'm leaving it out of the user guide.
...
\ccode{libdir}       & \ccode{libinfernal.a}   \\
\ccode{includedir}   & \ccode{infernal.h}      \\
...
\begin{comment}
% Currently builddirs don't work in Infernal, if fixed, uncomment
% this.
