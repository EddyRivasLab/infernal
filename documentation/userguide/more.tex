...
The appropriate citation is to the web site, \url{infernal.janelia.org}. You
...
quickly, do something like \prog{cmscann -h} to get a help page, and
the header will say:
...
\user{cmsearch --tblout tRNA5.mrum-genome.tbl tRNA5.cm mrum-genome.fa}
\user{esl-sfetch --index mrum-genome.fa}
\user{cat tRNA5.mrum-genome.tbl | grep -v ^\# | grep ! | awk '{ printf("%s/%s-%s %s %s %s\n", $1, $8, $9, $8, $9, $1); }' | esl-sfetch -Cf mrum-genome.fa - | ../src/cmalign tRNA5.cm -}
...
\prog{esl-sfetch} expects: <newname> <from> <to> <source seqname>.
These lines are then piped into \prog{esl-sfetch} (using the '-'
argument) to retrieve each hit (only the subsequence that comprises
each hit - not the full target sequence). \prog{esl-sfetch} will
output a FASTA file, which is finally being piped into \prog{cmalign},
again using the '-' argument. The output that is actually printed to
the screen will be a multiple alignment of all the included tRNA hits.

Ditto for piping subsets of CMs. Supposing you have a copy of Rfam in Rfam.cm:
...
database that is streamed into \prog{cmsearch}, which is not possible
because \prog{cmsearch} first reads the entire sequence file to
determine it's size (which dictates the filter thresholds that will be
