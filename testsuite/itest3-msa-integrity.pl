...
foreach $trial (1..5)
{
	...
	# random sequences
	`$builddir/easel/miniapps/easel synth -o $tmppfx.fa rna $n 50`;
	if ($? != 0) { die "FAIL: easel synth"; }
...
sub cmalign_msa_integrity_check
{
    ...
    `$builddir/easel/miniapps/easel reformat -r -u fasta $tmppfx.sto > $tmppfx.fa1 2>/dev/null`;
    if ($? != 0) { die "FAIL: first easel reformat failed"; }
    
    `$builddir/easel/miniapps/easel reformat -r -u fasta $fafile    > $tmppfx.fa2 2>/dev/null`;
    if ($? != 0) { die "FAIL: second easel reformat failed"; }
...
sub cmsearch_msa_integrity_check
{
    ...
    `$builddir/easel/miniapps/easel sindex -f $fafile > /dev/null 2>&1`;
    if ($? != 0) { die "FAIL: easel sindex failed"; }
    
    `cat $tmppfx.tbl | grep -v "^#" | awk '{print \$1, \$8, \$9, \$1}' > $tmppfx.gdf`;
    if ($? != 0) { die "FAIL: gdf table"; }

    `$builddir/easel/miniapps/easel sfetchn -C $fafile $tmppfx.gdf > $tmppfx.fa3`;
    if ($? != 0) { die "FAIL: easel sfetchn failed"; }

    # Grep out name/desc lines because they'll differ: esl-sfetch adds descline.
    `$builddir/easel/miniapps/easel reformat -r -u fasta $tmppfx.sto | grep -v "^>" > $tmppfx.fa1 2>/dev/null`;
    if ($? != 0) { die "FAIL: first easel reformat failed"; }
    
    `$builddir/easel/miniapps/easel reformat -r -u fasta $tmppfx.fa3 | grep -v "^>" > $tmppfx.fa2 2>/dev/null`;
    if ($? != 0) { die "FAIL: second easel reformat failed"; }
